...
export class ReplaySafetyGuard {
  ...
  check(actionType: string, payload: unknown): ReplaySafetyCheckResult {
    ...
    const redactedPayload = this.policy.redactSensitivePayloads ? this.redactSensitivePayload(payload) : payload
...
export function createReplaySafetyGuard(policy?: Partial<ReplaySafetyPolicy>): ReplaySafetyGuard {
