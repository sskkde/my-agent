...
function checkTerminalTransition(from: string, terminalStates: string[]): TransitionResult | null {
  if (terminalStates.includes(from)) {
    return createFailure(
      createError('INVALID_FROM_TERMINAL', `Cannot transition from terminal state ${from}`)
    );
  }
  return null;
}
...
const SUMMARY_TERMINAL = [
  SUMMARY_STATES.SUPERSEDED,
  SUMMARY_STATES.ARCHIVED,
  SUMMARY_STATES.EXPIRED,
];

const MEMORY_TERMINAL = [
...
export function validateExecutionPlanTransition(from: string, to: string): TransitionResult {
  ...
  if (allowed && !allowed.includes(to)) {
    return createFailure(
      createError('TRANSITION_NOT_ALLOWED', `Transition from ${from} to ${to} is not allowed`)
    );
  }
