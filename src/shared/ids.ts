...
export const ASK_ID_PREFIX = 'ask_'
...
export function isValidBackgroundRunId(id: string): boolean {
  return (
    typeof id === 'string' && id.startsWith(BACKGROUND_RUN_ID_PREFIX) && id.length > BACKGROUND_RUN_ID_PREFIX.length
  )
}
...
export function isValidWaitConditionId(id: string): boolean {
  return (
    typeof id === 'string' && id.startsWith(WAIT_CONDITION_ID_PREFIX) && id.length > WAIT_CONDITION_ID_PREFIX.length
  )
}
