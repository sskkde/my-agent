...
export class FailureAnalyzer {
  ...
  classifyFailure(error: Error): FailureCategory {
    ...
    if (errorMessage.includes('workflow') || errorMessage.includes('step') || errorName.includes('workflow')) {
      return 'workflow_step'
    }

    if (errorMessage.includes('watchdog') || errorMessage.includes('background') || errorName.includes('watchdog')) {
...
export function analyzeConnectorResponse(response: ConnectorResponse): FailureAnalysis {
  ...
  if (response.status === 'auth_required') {
    return analyzer.analyzeConnectorAuth(new Error('connector authentication required'))
  }

  if (response.status === 'rate_limited') {
    return analyzer.analyzeRateLimit(new Error('connector rate limit exceeded'))
  }
