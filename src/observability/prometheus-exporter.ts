...
class PrometheusExporterImpl implements PrometheusExporter {
  ...
  private formatCounter(fullName: string, metrics: MetricRecord[]): string[] {
    ...
    for (const metric of metrics) {
      const labels = mergeLabels(this.config.defaultLabels || {}, metric.labels)
  ...
  private formatHistogram(fullName: string, metrics: MetricRecord[]): string[] {
    ...
    for (const [, groupMetrics] of byLabels) {
      ...
      const baseLabels = mergeLabels(this.config.defaultLabels || {}, firstMetric.labels)
  ...
  private formatMetricLine(name: string, labelStr: string, value: number, timestamp?: string): string {
