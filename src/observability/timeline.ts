...
export class TimelineBuilder {
    ...
    switch (rootType) {
      ...
      case 'workflow_run':
        ...
        const filteredWorkflowEvents = workflowEvents.filter(
          (e) => e.relatedRefs?.workflowRunId === rootId
        );
        eventRecords = [...eventRecords, ...filteredWorkflowEvents];
        break;
      case 'background_run':
        const bgEvents = eventStore.query({});
        eventRecords = bgEvents.filter(
          (e) => e.relatedRefs?.backgroundRunId === rootId
        );
        break;
      case 'subagent_run':
        const subEvents = eventStore.query({});
        eventRecords = subEvents.filter(
          (e) => e.relatedRefs?.subagentRunId === rootId
        );
        break;
      case 'tool_call':
        const toolEvents = eventStore.query({});
        eventRecords = toolEvents.filter(
          (e) => e.relatedRefs?.toolCallId === rootId
        );
        break;
      case 'approval':
        const approvalEvents = eventStore.query({});
        eventRecords = approvalEvents.filter(
          (e) => e.relatedRefs?.approvalId === rootId
        );
        break;
      case 'memory':
        const memEvents = eventStore.query({});
        eventRecords = memEvents.filter(
          (e) => e.relatedRefs?.memoryId === rootId
        );
        break;
    }
  ...
  addAuditsFromStore(
    ...
    for (const record of auditRecords) {
      const existingEvent = events.find(
        (e) => e.eventType === 'audit' && e.eventId === record.auditId
      );
      if (!existingEvent) {
  ...
  addAuditRefs(events: TimelineEvent[]): void {
    ...
    for (const event of events) {
      ...
      for (const audit of auditRecords) {
        ...
        const existingEvent = events.find(
          (e) => e.eventType === 'audit' && e.eventId === audit.auditId
        );
    ...
    for (const event of events) {
      ...
      for (const trace of traces) {
        ...
        for (const span of spans) {
          ...
          const existingEvent = events.find(
            (e) => e.eventType === 'span' && e.eventId === span.spanId
          );
  ...
  addActionRefs(
    ...
    for (const action of actions) {
      ...
      const existingEvent = events.find(
        (e) => e.eventType === 'action' && e.eventId === action.actionId
      );
