...
export class RetentionPolicy {
  ...
  private countTotalTraces(): number {
    const rows = this.connection.query<{ count: number }>('SELECT COUNT(*) as count FROM trace_contexts')
    return rows[0]?.count ?? 0
  }
  ...
  private applyTraceRetention(cutoffTimestamp: string): number {
    ...
    this.connection.exec('DELETE FROM trace_contexts WHERE started_at < ?', [cutoffTimestamp])
  ...
  private countTotalMetrics(): number {
    const rows = this.connection.query<{ count: number }>('SELECT COUNT(*) as count FROM metrics')
    return rows[0]?.count ?? 0
  }

  private countEligibleMetrics(cutoffTimestamp: string): number {
    const rows = this.connection.query<{ count: number }>('SELECT COUNT(*) as count FROM metrics WHERE timestamp < ?', [
      cutoffTimestamp,
    ])
    return rows[0]?.count ?? 0
  }
  ...
  private applyMetricRetention(cutoffTimestamp: string): number {
    this.connection.exec('DELETE FROM metrics WHERE timestamp < ?', [cutoffTimestamp])
  ...
  private countTotalBlobs(): number {
    const rows = this.connection.query<{ count: number }>('SELECT COUNT(*) as count FROM tool_results')
    return rows[0]?.count ?? 0
  }
