...
export class MetricsRollup {
  ...
  private calculateConnectorCallMetrics(spans: SpanRow[]): ConnectorCallMetrics {
    ...
    for (const span of connectorSpans) {
      ...
      if (span.status === 'completed') {
        ...
        failureCount++;
        if (metadata.errorCategory === 'rate_limited' || 
            span.error?.includes('rate') ||
            span.error?.includes('429')) {
          rateLimitedCount++;
        }
      }
    }
  ...
  private calculateApprovalMetrics(spans: SpanRow[]): ApprovalMetrics {
    const approvalSpans = spans.filter(s => 
      s.span_type === 'permission_check' || 
      s.module === 'permission'
    );
  ...
  private calculateRetryMetrics(metrics: MetricRow[]): RetryMetrics {
    const retryMetrics = metrics.filter(m => 
      m.name.includes('retry') || m.name.includes('attempt')
    );
    ...
    const avgAttemptsPerOperation = attemptsPerOp.length > 0
      ? attemptsPerOp.reduce((a, b) => a + b, 0) / attemptsPerOp.length
      : 1;
  ...
  private categorizeError(span: SpanRow, metadata: Record<string, unknown>): FailureCategory {
    ...
    if (errorCategory === 'auth_error' || 
        span.error?.includes('401') ||
        span.error?.includes('403')) {
      return 'connector_auth_error';
    }
    
    if (errorCategory === 'permission_denied' || 
        span.module === 'permission' ||
        span.error?.includes('permission')) {
