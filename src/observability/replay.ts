...
export class ReplayService {
  ...
  preserveTraceRefs(timeline: RuntimeTimeline): OriginalTraceRef[] {
    ...
    for (const event of timeline.events) {
      ...
      const sourceData = event.sourceData as
        | EventRecord
        | AuditRecord
        | RuntimeSpan
        | undefined;
  ...
  private queryStateChangingEvents(
    eventStore: EventStore,
    ...
    return uniqueEvents.filter((event) =>
      stateChangingEventTypes.includes(event.eventType)
    );
  }

  private reconstructWorkflowRunState(
    events: EventRecord[],
    workflowRunId: string
  ): WorkflowRunState {
  ...
  private reconstructBackgroundRunState(
    events: EventRecord[],
    backgroundRunId: string
  ): BackgroundRunState {
    let status = 'unknown';
  ...
  private reconstructPlannerRunState(
    events: EventRecord[],
    plannerRunId: string
  ): PlannerRunState {
    let status = 'unknown';
  ...
  private extractCorrelationId(
    sourceData: EventRecord | AuditRecord | RuntimeSpan
  ): string | undefined {
