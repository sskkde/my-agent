...
function appendOptionalField(value: string | undefined, role: LLMMessage['role'], messages: LLMMessage[]): void {
  if (value) {
    messages.push({ role, content: value })
  }
}

function appendSummaryBlocks(summaryBlocks: ContextItemData[] | undefined, messages: LLMMessage[]): void {
...
function appendTranscript(transcript: LLMMessage[] | undefined, messages: LLMMessage[]): void {
...
function appendDynamicFields(fields: DynamicFields, messages: LLMMessage[]): void {
  const parts: string[] = []
