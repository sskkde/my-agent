...
export interface CancelModifyData {
  runtimeActionId: string;
  actionType: 'cancel_planner_run' | 'cancel_background_subagent' | 'pause_planner_run' | 'resume_planner_run' | 'pause_background_subagent' | 'resume_background_subagent';
  ...
  try {
    ...
    if (isPlannerRun && input.plannerRunId) {
      ...
      if (!run) {
        return createErrorResult(
          'TASK_NOT_FOUND',
          `Subagent run not found: ${input.runtimeActionId}`,
          true
        );
      }
      if (run.userId !== deps.userId) {
        return createErrorResult(
          'UNAUTHORIZED_CANCEL',
          `Cannot cancel subagent run belonging to another user`,
          false
        );
      }
    }
