...
import { PLANNER_STATES, BACKGROUND_SUBAGENT_STATES, WORKFLOW_RUN_STATES } from '../shared/states.js'
import type { ActiveWorkProjection, ProjectionSource, ProjectionCache, PartialActiveWorkProjection } from './types.js'
...
class ActiveWorkProjectionBuilderImpl implements ActiveWorkProjectionBuilder {
   ...
   */
  private getActiveBackgroundRuns(
    userId: string,
  ): Array<{ runId: string; status: string; startedAt: string; objective?: string }> {
    const allStatuses = Object.values(BACKGROUND_SUBAGENT_STATES) as BackgroundSubagentState[]
    const activeStatuses = allStatuses.filter((status) => !TERMINAL_BACKGROUND_STATES.includes(status))
   ...
   * Queries each non-terminal status individually and combines results.
   */
  private getActiveWorkflowRuns(
    userId: string,
  ): Array<{ runId: string; status: string; startedAt: string; objective?: string }> {
    const allStatuses = Object.values(WORKFLOW_RUN_STATES) as WorkflowRunState[]
    const activeStatuses = allStatuses.filter((status) => !TERMINAL_WORKFLOW_STATES.includes(status))
  ...
  async buildProjection(userId: string, options: BuildProjectionOptions = {}): Promise<ActiveWorkProjection> {
    ...
    const activePlannerRuns = includeTerminal
      ...
      : (merged.activePlannerRuns ?? []).filter((run) => !TERMINAL_PLANNER_STATES.includes(run.status))
  ...
  private mergePartialResults(partials: PartialActiveWorkProjection[]): PartialActiveWorkProjection {
