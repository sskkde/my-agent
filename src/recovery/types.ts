...
export interface ToolExecutionStore {
  ...
  saveResult: (
    toolCallId: string,
    result: { synthetic?: boolean; status?: string; reason?: string; error?: { code: string; message: string } },
  ) => void
}

export interface PlannerRunStore {
  getById: (plannerRunId: string) => {
    plannerRunId: string
    status: string
    checkpoint?: { activeExecutionRefs?: Array<{ refId: string; refType: string; status: string }> }
  } | null
...
export type RecoverableRunType =
  | 'planner_run'
  | 'kernel_run'
  | 'workflow_run'
  | 'workflow_step_run'
...
export interface RecoveryEventStore {
  findByCorrelationId: (
    correlationId: string,
  ) => Array<{ eventId: string; eventType: string; createdAt: string; payload: Record<string, unknown> }>
}
