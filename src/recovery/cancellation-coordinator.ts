...
class CancellationCoordinatorImpl implements CancellationCoordinator {
  ...
  async cancelWorkflowRun(workflowRunId: string): Promise<CancellationResult> {
    ...
    return {
      status:
        failedRefs.length > 0 || partialRefs.length > 0
          ? CANCELLATION_STATUSES.PARTIAL
          : CANCELLATION_STATUSES.COMPLETED,
      affectedRefs,
      failedRefs,
  ...
  private async cancelRef(ref: {
    refId: string
    refType: string
    status: string
  }): Promise<{ cancelled: boolean; alreadyTerminal: boolean; hasExternalSideEffects: boolean }> {
    const terminalStatuses = ['completed', 'failed', 'cancelled']
    ...
    switch (ref.refType) {
      ...
      case 'approval_request': {
        const result = await this.cancelTerminalStoreRef(
          ref.refId,
          'approval_request_cancelled',
          this.config.approvalRequestStore,
        )
        return {
          cancelled: result.status === CANCELLATION_STATUSES.COMPLETED,
          alreadyTerminal: result.status === CANCELLATION_STATUSES.ALREADY_TERMINAL,
      ...
      case 'wait_condition': {
        const result = await this.cancelTerminalStoreRef(
          ...
          this.config.waitConditionStore,
        )
        return {
          cancelled: result.status === CANCELLATION_STATUSES.COMPLETED,
          alreadyTerminal: result.status === CANCELLATION_STATUSES.ALREADY_TERMINAL,
      ...
      case 'connector_operation': {
        const result = await this.cancelTerminalStoreRef(
          ref.refId,
          'connector_operation_cancelled',
          this.config.connectorOperationStore,
        )
        return {
          cancelled: result.status === CANCELLATION_STATUSES.COMPLETED,
          alreadyTerminal: result.status === CANCELLATION_STATUSES.ALREADY_TERMINAL,
  ...
  private async cancelTerminalStoreRef(
    refId: string,
    eventType: string,
    store?: { getById: (id: string) => { status: string } | null; updateStatus: (id: string, status: string) => void },
  ...
  private getTransitionStore(
    targetType: string,
  ):
    | { getById: (id: string) => { status: string } | null; updateStatus: (id: string, status: string) => void }
    | undefined {
    switch (targetType) {
      case CANCELLATION_TARGET_TYPES.PLANNER_RUN:
  ...
  private getWorkflowStepChildRefs(stepRun: {
    status: string
    kernelRunId?: string
    subagentRunId?: string
    toolCallId?: string
    approvalId?: string
  }): Array<{ refId: string; refType: string; status: string }> {
    ...
    if (stepRun.subagentRunId)
      refs.push({ refId: stepRun.subagentRunId, refType: 'background_run', status: stepRun.status })
    if (stepRun.toolCallId) refs.push({ refId: stepRun.toolCallId, refType: 'tool_execution', status: stepRun.status })
    if (stepRun.approvalId)
      refs.push({ refId: stepRun.approvalId, refType: 'approval_request', status: stepRun.status })
    return refs
  }
