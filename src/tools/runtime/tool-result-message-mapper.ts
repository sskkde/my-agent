...
function generateUserVisibleSummary(result: unknown, isError: boolean): string {
  ...
  if (typeof result === 'string') {
    return result.length > 200 ? `string result (${result.length} chars)` : result
  }
...
export function mapToolResultToMessage(result: ToolUseResult, options?: MapToolResultOptions): ToolResultMessage {
