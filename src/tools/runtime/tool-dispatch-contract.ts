...
export type ToolDispatchStatus =
  | 'completed'
  | 'partial'
  | 'failed'
  | 'cancelled';
...
export function createToolDispatchRequest(
  ...
  return {
    ...
    executionPolicy: {
      ...
      ...(fields.executionPolicy?.abortOnSiblingFailure !== undefined ? { abortOnSiblingFailure: fields.executionPolicy.abortOnSiblingFailure } : {}),
    },
  };
}
...
export function isTerminalStatus(status: ToolDispatchStatus): status is Extract<ToolExecutionTerminalStatus, ToolDispatchStatus> {
