...
export function createResultRef(
  ...
  const preview = serialized.length > 1000 ? serialized.substring(0, 1000) + '...' : serialized
  ...
  const blob: Omit<ToolResultBlob, 'id' | 'createdAt'> = {
    ...
    structuredContent:
      typeof output === 'object' && output !== null ? (output as Record<string, unknown>) : { value: output },
