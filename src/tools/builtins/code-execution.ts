...
export function createCodeExecutionTool(store: ProcessSessionStore): ToolDefinition {
      ...
      const checkInterval = setInterval(() => {
        ...
        if (session.status !== 'running') {
          ...
          const execResult: CodeExecutionResult = {
            ...
            status: session.status === 'timeout' ? 'timeout' : session.status === 'failed' ? 'failed' : 'completed',
  ...
  return {
    name: 'code_execution',
    description:
      'Execute code in JavaScript (always available), TypeScript (requires tsx), or Bash. Creates temp files in workspace, runs with timeout, and cleans up. NOT a sandbox - runs in the same environment as the agent.',
    category: 'execute',
    sensitivity: 'high',
