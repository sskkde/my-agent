 ...
 */
export function validateSymlinkSafety(path: string, workspaceRoot?: string): PathSafetyResult {
  const root = workspaceRoot ?? getWorkspaceRoot()
 ...
 */
export function normalizePathForDisplay(path: string, workspaceRoot?: string): string {
  const root = workspaceRoot ?? getWorkspaceRoot()
  const canonicalPath = resolveCanonicalPath(path, root)
