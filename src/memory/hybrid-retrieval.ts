...
export class HybridRetrievalOrchestrator {
  ...
  async recall(query: HybridRecallQuery): Promise<HybridRecallResult> {
    ...
    const indexItems = isHybridRetrievalEnabled() ? this.queryEntityTimeIndexes(query) : []

    const mergedWithIndex = mergeResults(lexicalResult.items, indexItems)

    if (mergedWithIndex.length >= minResults || !vector) {
      const limited = mergedWithIndex.sort((a, b) => b.relevanceScore - a.relevanceScore).slice(0, limit)
      return {
        items: limited,
    ...
    const limited = merged.sort((a, b) => b.relevanceScore - a.relevanceScore).slice(0, limit)
...
function mergeResults(primaryItems: HybridRecallItem[], secondaryItems: HybridRecallItem[]): HybridRecallItem[] {
