...
export function createMemorySearch(summaryStore: SummaryStore): MemorySearchType {
  ...
  function search(options: MemorySearchOptions): MemorySearchResult[] {
    const allSummaries = summaryStore.getByType('working_summary')
    ...
    if (options.summaryTypes && options.summaryTypes.length > 0) {
      results = results.filter(r =>
        options.summaryTypes!.includes(r.record.summaryType as SummaryType)
      );
    }

    if (options.statuses && options.statuses.length > 0) {
      results = results.filter(r =>
        options.statuses!.includes(r.record.status as SummaryStatus)
      );
    }

    if (options.importance) {
      results = results.filter(r =>
        r.record.retrieval?.importance === options.importance
      );
    }
  ...
  function matchesSourceRefs(
    recordSourceRefs: SourceRefs,
    searchSourceRefs: SourceRefs
  ): boolean {
    if (searchSourceRefs.transcriptRefs && searchSourceRefs.transcriptRefs.length > 0) {
      ...
      const hasMatch = searchSourceRefs.transcriptRefs.some(ref =>
        recordRefs.includes(ref)
      );
    ...
    if (searchSourceRefs.eventRange) {
      ...
      if (recordRange) {
        ...
        if (
          recordRange.startEventId === searchStart &&
          recordRange.endEventId === searchEnd
        ) {
          return true;
        }
      }
    ...
    if (searchSourceRefs.previousSummaryRefs && searchSourceRefs.previousSummaryRefs.length > 0) {
      ...
      const hasMatch = searchSourceRefs.previousSummaryRefs.some(ref =>
        recordRefs.includes(ref)
      );
      if (hasMatch) return true;
    }
