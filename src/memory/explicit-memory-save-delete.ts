...
export function createExplicitMemoryService(store: LongTermMemoryStore): ExplicitMemoryService {
  return {
    saveMemory(userId, content, metadata, sourceRefs) {
      ...
      if (
        store.hasTombstone(userId, fingerprint, sourceWindowHash) ||
        store.hasTombstoneForSource(userId, sourceWindowHash)
      ) {
...
function validateSourceRefs(sourceRefs: MemorySourceRefs): void {
  ...
  const hasOtherRef = Boolean(
    sourceRefs.workflowRunId ||
    sourceRefs.backgroundRunId ||
    sourceRefs.artifactId ||
    sourceRefs.extraction?.windowHash,
  )
