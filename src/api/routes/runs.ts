...
export function registerRunRoutes(server: FastifyInstance, context: ApiContext): void {
  server.get('/api/v1/runs', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    try {
      const backgroundRuns = context.stores.backgroundRunStore.getByUserAndStatus(
        'default',
        'running' as BackgroundSubagentState,
      )
      const pendingRuns = context.stores.backgroundRunStore.getByUserAndStatus(
        'default',
        'pending' as BackgroundSubagentState,
      )
