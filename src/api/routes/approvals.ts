  ...
  server.patch<{ Params: { approvalId: string }; Body: ApprovalDecisionRequest }>(
    ...
    async (request: FastifyRequest<{ Params: { approvalId: string }; Body: ApprovalDecisionRequest }>, reply: FastifyReply) => {
      ...
      if (responseType) {
        ...
        return reply.code(400).send(envelopeError('BAD_REQUEST', 'Either responseType or decision is required', request.requestId));
      }

      if (!['reject', 'approve_once', 'approve_always'].includes(effectiveResponseType)) {
        return reply.code(400).send(envelopeError('BAD_REQUEST', `Invalid responseType. Must be "reject", "approve_once", or "approve_always"`, request.requestId));
      }
      ...
      if (effectiveResponseType === 'reject') {
        ...
        const ttlMs = effectiveResponseType === 'approve_once' 
