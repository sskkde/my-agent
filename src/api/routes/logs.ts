...
function createPayloadPreview(event: EventRecord, sensitivity: SensitivityLevel): string | undefined {
  if (sensitivity !== 'low') {
    return '[redacted]'
  }
...
function mapEventToLogEntry(event: EventRecord): LogEntry {
  const summary =
    event.payload.message && typeof event.payload.message === 'string'
...
export function registerLogRoutes(server: FastifyInstance, context: ApiContext): void {
  server.get<{
    ...
    const { sessionId, sourceModule, eventType, runRef, limit: limitStr, offset: offsetStr } = request.query
  ...
  server.get<{
    ...
    '/api/v1/logs/stream',
    async (request: FastifyRequest<{ Querystring: LogStreamQueryParams }>, reply: FastifyReply) => {
      if (!request.requirePermission(ResourceType.observability, Action.read)) {
        return reply
      }
      ...
      const logs = filteredEvents.filter((e) => e.sensitivity === 'low').map(mapEventToLogEntry)
