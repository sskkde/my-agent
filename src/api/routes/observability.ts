...
export function registerObservabilityRoutes(server: FastifyInstance, context: ApiContext): void {
      ...
      try {
        ...
        const consoleView: ConsoleView = {
          ...
          status: {
            ...
            startedAt:
              'startedAt' in (run as unknown as Record<string, unknown>)
                ? ((run as unknown as Record<string, unknown>).startedAt as string | undefined)
                : undefined,
            completedAt:
              'completedAt' in (run as unknown as Record<string, unknown>)
                ? ((run as unknown as Record<string, unknown>).completedAt as string | undefined)
                : undefined,
            errorMessage:
              'errorMessage' in (run as unknown as Record<string, unknown>)
                ? ((run as unknown as Record<string, unknown>).errorMessage as string | undefined)
                : undefined,
          },
        }
  ...
  // --------------------------------------------------------------------------
  server.get('/api/v1/metrics', async (_request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      return reply.code(200).header('Content-Type', 'text/plain; version=0.0.4; charset=utf-8').send(output)
    } catch (err) {
  ...
  server.get('/api/v1/alerts/rules', async (request: FastifyRequest, reply: FastifyReply) => {
    if (!request.requirePermission(ResourceType.observability, Action.read)) {
      return reply
    }
    try {
  ...
  server.get('/api/v1/alerts/state', async (request: FastifyRequest, reply: FastifyReply) => {
    if (!request.requirePermission(ResourceType.observability, Action.read)) {
      return reply
    }
    try {
  ...
  server.post('/api/v1/alerts/evaluate', async (request: FastifyRequest, reply: FastifyReply) => {
