...
const VALID_FALLBACK_MODES: SubagentProviderPreference['fallbackMode'][] = [
  'none',
  'same_provider',
  'any_compatible',
];
...
export function registerSubagentRoutes(server: FastifyInstance, context: ApiContext): void {
  ...
  server.get<{ Params: { agentType: string } }>(
    ...
    async (request: FastifyRequest<{ Params: { agentType: string } }>, reply: FastifyReply) => {
      ...
      if (!definition) {
        return reply.code(404).send(envelopeError('NOT_FOUND', `Subagent type "${agentType}" not found`, request.requestId));
      }
  ...
  server.put<{ Params: { agentType: string }; Body: SetPreferenceRequest }>(
    '/api/v1/subagents/:agentType/preference',
    async (request: FastifyRequest<{ Params: { agentType: string }; Body: SetPreferenceRequest }>, reply: FastifyReply) => {
