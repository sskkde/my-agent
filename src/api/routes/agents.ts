...
function mergeConfigs(global: AgentConfig | null, userOverride: AgentConfig | null): Partial<AgentConfig> | null {
  ...
  return {
    ...
    routingTimeoutMs:
      userOverride.routingTimeoutMs === INHERIT_ROUTING_TIMEOUT_MS
        ? (global?.routingTimeoutMs ?? DEFAULT_ROUTING_TIMEOUT_MS)
        : userOverride.routingTimeoutMs,
    repairAttempts:
      userOverride.repairAttempts === INHERIT_REPAIR_ATTEMPTS
...
function validateConfigInput(
  ...
  if (input.displayName !== undefined) {
    if (typeof input.displayName !== 'string' || input.displayName.trim().length === 0) {
      return {
        valid: false,
        error: { code: 'INVALID_DISPLAY_NAME', message: 'Display name must be a non-empty string' },
      }
    }
    if (input.displayName.length > 100) {
      return {
        valid: false,
        error: { code: 'DISPLAY_NAME_TOO_LONG', message: 'Display name must be 100 characters or less' },
      }
    }
  }
  ...
  if (input.systemPrompt !== undefined && input.systemPrompt !== null) {
    if (typeof input.systemPrompt !== 'string') {
      return {
        valid: false,
        error: { code: 'INVALID_SYSTEM_PROMPT', message: 'System prompt must be a string or null' },
      }
    }
    if (input.systemPrompt.length > MAX_PROMPT_LENGTH) {
      return {
        ...
        error: {
          ...
          message: `System prompt must be ${MAX_PROMPT_LENGTH} characters or less`,
        },
      }
    }
  }
  ...
  if (input.routingPrompt !== undefined) {
    if (input.routingPrompt !== null && typeof input.routingPrompt !== 'string') {
      return {
        valid: false,
        error: { code: 'INVALID_ROUTING_PROMPT', message: 'Routing prompt must be a string or null' },
      }
    }
    if (typeof input.routingPrompt === 'string' && input.routingPrompt.length > MAX_PROMPT_LENGTH) {
      return {
        valid: false,
        error: {
          code: 'ROUTING_PROMPT_TOO_LONG',
          message: `Routing prompt must be ${MAX_PROMPT_LENGTH} characters or less`,
        },
      }
    }
  }
  ...
  if (input.allowedToolIds !== undefined && input.allowedToolIds !== null) {
    ...
    if (invalidTools.length > 0) {
      return {
        valid: false,
        error: { code: 'INVALID_TOOL_ID', message: `Invalid tool IDs: ${invalidTools.join(', ')}` },
      }
    }
  }
  ...
  if (input.allowedSkillIds !== undefined && input.allowedSkillIds !== null) {
    ...
    if (invalidSkills.length > 0) {
      return {
        valid: false,
        error: { code: 'INVALID_SKILL_ID', message: `Invalid skill IDs: ${invalidSkills.join(', ')}` },
      }
    }
  }
    ...
    if (input.routingTimeoutMs < MIN_TIMEOUT_MS || input.routingTimeoutMs > MAX_TIMEOUT_MS) {
      return {
        ...
        error: {
          ...
          message: `Timeout must be between ${MIN_TIMEOUT_MS} and ${MAX_TIMEOUT_MS} milliseconds`,
        },
      }
    }
  }
  ...
  if (input.repairAttempts !== undefined) {
    ...
    if (!VALID_REPAIR_ATTEMPTS.includes(input.repairAttempts)) {
      return {
        valid: false,
        error: { code: 'REPAIR_ATTEMPTS_OUT_OF_RANGE', message: `Repair attempts must be 0 or 1 for V1` },
      }
    }
  }
...
export function registerAgentRoutes(server: FastifyInstance, context: ApiContext): void {
  ...
  server.patch<{ Params: { agentId: string }; Body: UpdateGlobalConfigRequest }>(
    ...
    async (
      request: FastifyRequest<{ Params: { agentId: string }; Body: UpdateGlobalConfigRequest }>,
      reply: FastifyReply,
    ) => {
      const userId = request.user?.userId
      if (!userId) {
      ...
      try {
        ...
        const config = agentConfigStore.upsert({
          ...
          searchLlmProviderId:
            searchLlmProviderId !== undefined
              ? searchLlmProviderId
              : (existingGlobal?.searchLlmProviderId ?? undefined),
  ...
  server.patch<{ Params: { agentId: string }; Body: UpdateOverrideConfigRequest }>(
    ...
    async (
      request: FastifyRequest<{ Params: { agentId: string }; Body: UpdateOverrideConfigRequest }>,
      reply: FastifyReply,
    ) => {
      const userId = request.user?.userId
      if (!userId) {
      ...
      try {
        ...
        const config = agentConfigStore.upsert({
          ...
          systemPrompt: systemPrompt !== undefined ? systemPrompt : (existingOverride?.systemPrompt ?? null),
          routingPrompt: routingPrompt !== undefined ? routingPrompt : (existingOverride?.routingPrompt ?? null),
          providerId: providerId !== undefined ? providerId : (existingOverride?.providerId ?? null),
          model: model !== undefined ? model : (existingOverride?.model ?? null),
          allowedToolIds: allowedToolIds !== undefined ? allowedToolIds : (existingOverride?.allowedToolIds ?? null),
          allowedSkillIds:
            allowedSkillIds !== undefined ? allowedSkillIds : (existingOverride?.allowedSkillIds ?? null),
          ...
          searchLlmProviderId:
            searchLlmProviderId !== undefined ? searchLlmProviderId : (existingOverride?.searchLlmProviderId ?? null),
          searchLlmModel: searchLlmModel !== undefined ? searchLlmModel : (existingOverride?.searchLlmModel ?? null),
        })
