...
export function registerConnectorRoutes(server: FastifyInstance, context: ApiContext): void {
  ...
  server.get('/api/v1/connectors', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    const definitions = ALL_CONNECTOR_TYPES.flatMap((type) => connectorStore.findDefinitionsByType(type))
  ...
  server.patch<{ Params: { id: string; iid: string }; Body: UpdateInstanceConfigRequest }>(
    ...
    async (
      request: FastifyRequest<{ Params: { id: string; iid: string }; Body: UpdateInstanceConfigRequest }>,
      reply: FastifyReply,
    ) => {
      ...
      if (!updated) {
        return reply
          .code(500)
          .send(envelopeError('INTERNAL_ERROR', 'Failed to update instance config', request.requestId))
      }
