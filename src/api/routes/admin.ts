...
function toAdminUser(user: { userId: string; username: string; role: UserRole; createdAt: string; updatedAt: string }, status: 'active' | 'disabled' = 'active') {
  return {
    userId: user.userId,
...
export function registerAdminRoutes(server: FastifyInstance, context: ApiContext): void {
  ...
  server.patch<{ Params: { userId: string }; Body: { role: UserRole } }>(
    ...
    async (request: FastifyRequest<{ Params: { userId: string }; Body: { role: UserRole } }>, reply: FastifyReply) => {
      ...
      return reply.code(200).send(success({ user: toAdminUser({ ...existing, role, updatedAt: now }) }, request.requestId));
    },
  );

  server.patch<{ Params: { userId: string }; Body: { status: 'active' | 'disabled' } }>(
    '/api/v1/admin/users/:userId/status',
    async (request: FastifyRequest<{ Params: { userId: string }; Body: { status: 'active' | 'disabled' } }>, reply: FastifyReply) => {
      if (!request.requirePermission(ResourceType.users, Action.manage)) {
        return reply;
      }
  ...
  server.get('/api/v1/admin/settings', async (request: FastifyRequest, reply: FastifyReply) => {
    ...
    return reply.code(200).send(success({ settings: { rateLimitPerMinute: 60, rateLimitPerHour: 1000, sessionTokenTtlHours: 24 } }, request.requestId));
  });

  server.patch<{ Body: { rateLimitPerMinute?: number; rateLimitPerHour?: number; sessionTokenTtlHours?: number } }>(
    '/api/v1/admin/settings',
    async (request: FastifyRequest<{ Body: { rateLimitPerMinute?: number; rateLimitPerHour?: number; sessionTokenTtlHours?: number } }>, reply: FastifyReply) => {
