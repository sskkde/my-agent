...
export function resolveProviderAndModel(
  ...
  const usableUserProviders = userProviders.filter(
  ...
  if (session.selectedProviderId) {
    const sessionProvider = userProviders.find(
      (p) => p.providerId === session.selectedProviderId
    );
    candidates.push({
  ...
  if (agentConfig.providerId) {
    const agentProvider = userProviders.find(
      (p) => p.providerId === agentConfig.providerId
    );
