...
const TERMINAL_STATES: PlannerRunState[] = [PLANNER_STATES.COMPLETED, PLANNER_STATES.FAILED, PLANNER_STATES.CANCELLED]
...
class PlannerRuntimeImpl implements PlannerRuntime {
  ...
  private emitPlannerStatePatch(plannerRunId: string, data: PlannerStatePatchData): void {
    const now = new Date().toISOString()
