    ...
    switch (reason) {
      case 'approval_rejected':
        if (options.hasAlternativePaths) {
          return buildReplan('approval_rejected', 'Approval was rejected. Replanning with alternative paths.', true)
        }
      ...
      case 'tool_failed':
        if (options.isRetryable) {
          return buildReplan('tool_failed', 'Tool execution failed but is retryable. Replanning with retry.', true)
        }
        return buildTerminate('tool_failed', 'Tool execution failed and is not retryable. Terminating plan execution.')
      ...
      case 'timeout':
        return buildWaitForUser('timeout', 'Plan execution timed out. Waiting for user input on how to proceed.')
