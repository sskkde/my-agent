...
function buildFinalResponseStep(
  id: string,
  dependsOn?: PlanDependency[],
): PlanStep {
  return {
...
export class DeterministicPlanGenerator implements PlanGenerator {
  ...
  private generateSteps(
    ...
    if (!complex && effectiveMax <= 2) {
      ...
      const finalDep: PlanDependency[] = [
        { type: 'depends_on', targetStepId: step1.id },
      ];
    ...
    if (complex && hasWriteIntent && effectiveMax >= 3) {
      ...
      const approvalDep: PlanDependency[] = [
        { type: 'depends_on', targetStepId: step1.id },
      ];
      ...
      if (effectiveMax >= 4) {
        const execDep: PlanDependency[] = [
          { type: 'depends_on', targetStepId: approvalStep.id },
        ];
        ...
        const finalDep: PlanDependency[] = [
          { type: 'depends_on', targetStepId: execStep.id },
        ];
        steps.push(buildFinalResponseStep(nextId(stepIndex++), finalDep));
      } else {
        const finalDep: PlanDependency[] = [
    ...
    if (complex && !hasWriteIntent && effectiveMax >= 3) {
      ...
      if (effectiveMax >= 4) {
        ...
        const finalDep: PlanDependency[] = [
          { type: 'depends_on', targetStepId: step2.id },
        ];
        steps.push(buildFinalResponseStep(nextId(stepIndex++), finalDep));
      } else {
