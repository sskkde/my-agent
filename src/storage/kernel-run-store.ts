...
class KernelRunStoreImpl implements KernelRunStore {
  ...
  getById(runId: string): KernelRun | null {
    const results = this.connection.query<{
      ...
      created_at: string
      updated_at: string
    }>(`SELECT * FROM kernel_runs WHERE run_id = ?`, [runId])
  ...
  updateStatus(runId: string, status: KernelRunState): void {
    const now = new Date().toISOString()
    this.connection.exec(`UPDATE kernel_runs SET status = ?, updated_at = ? WHERE run_id = ?`, [status, now, runId])
  }

  saveCheckpoint(runId: string, checkpoint: unknown): void {
    const now = new Date().toISOString()
    this.connection.exec(`UPDATE kernel_runs SET checkpoint_data = ?, updated_at = ? WHERE run_id = ?`, [
  ...
  saveFinalResult(runId: string, result: unknown): void {
    const now = new Date().toISOString()
    this.connection.exec(`UPDATE kernel_runs SET final_result = ?, updated_at = ? WHERE run_id = ?`, [
  ...
  getBySession(sessionId: string): KernelRun[] {
    const results = this.connection.query<{
      ...
      created_at: string
      updated_at: string
    }>(`SELECT * FROM kernel_runs WHERE session_id = ? ORDER BY created_at DESC`, [sessionId])
  ...
  getByAgentId(agentId: string): KernelRun[] {
    const results = this.connection.query<{
      ...
      created_at: string
      updated_at: string
    }>(`SELECT * FROM kernel_runs WHERE agent_id = ? ORDER BY created_at DESC`, [agentId])
  ...
  getByStatus(status: KernelRunState): KernelRun[] {
    const results = this.connection.query<{
      ...
      created_at: string
      updated_at: string
    }>(`SELECT * FROM kernel_runs WHERE status = ? ORDER BY created_at DESC`, [status])
  ...
  getByParentRunId(parentRunId: string): KernelRun[] {
    const results = this.connection.query<{
      ...
      created_at: string
      updated_at: string
    }>(`SELECT * FROM kernel_runs WHERE parent_run_id = ? ORDER BY created_at DESC`, [parentRunId])
  ...
  getByRootRunId(rootRunId: string): KernelRun[] {
    const results = this.connection.query<{
      ...
      created_at: string
      updated_at: string
    }>(`SELECT * FROM kernel_runs WHERE root_run_id = ? ORDER BY created_at DESC`, [rootRunId])
