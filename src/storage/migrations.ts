...
class MigrationRunnerImpl implements MigrationRunner {
  ...
  getCurrentVersion(): number {
    try {
      const result = this.connection.query<{ version: number }>('SELECT MAX(version) as version FROM migrations')
  ...
  apply(migrations: Migration[]): void {
    ...
    for (const migration of sortedMigrations) {
      ...
      this.connection.transaction(() => {
        ...
        this.connection.exec('INSERT INTO migrations (version, name, applied_at, checksum) VALUES (?, ?, ?, ?)', [
          migration.version,
          migration.name,
    ...
    for (const statement of statements) {
      try {
        ...
        if (upperStatement.startsWith('CREATE TABLE')) {
          const tableMatch = statement.trim().match(/CREATE\s+TABLE\s+(?:IF\s+NOT\s+EXISTS\s+)?['"]?(\w+)['"]?/i)
          if (tableMatch) {
  ...
  private ensureTenantIdColumn(tableName: string): void {
    const columns = this.connection.query<{ name: string }>(`PRAGMA table_info('${tableName}')`)
    const hasTenantId = columns.some((c) => c.name === 'tenant_id')
    if (!hasTenantId) {
      try {
        this.connection.exec(`ALTER TABLE "${tableName}" ADD COLUMN tenant_id TEXT NOT NULL DEFAULT 'org_default'`)
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err)
