...
class WorkflowRunStoreImpl implements WorkflowRunStore {
  ...
  getWorkflowRunById(workflowRunId: string, tenantId: string = DEFAULT_TENANT_ID): WorkflowRun | null {
    const results = this.connection.query<{
      ...
      created_at: string
      updated_at: string
    }>(`SELECT * FROM workflow_runs WHERE tenant_id = ? AND workflow_run_id = ?`, [tenantId, workflowRunId])
  ...
  getWorkflowRunsByOwnerAndStatus(
    ownerUserId: string,
    status: WorkflowRunState,
  ...
  getStepRunById(stepRunId: string, tenantId: string = DEFAULT_TENANT_ID): WorkflowStepRun | null {
    const results = this.connection.query<{
      ...
      created_at: string
      updated_at: string
    }>(`SELECT * FROM workflow_step_runs WHERE tenant_id = ? AND step_run_id = ?`, [tenantId, stepRunId])
