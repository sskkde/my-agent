...
class PermissionGrantStoreImpl implements PermissionGrantStore {
  ...
  getById(id: string): PermissionGrant | null {
    const results = this.connection.query<PermissionGrantRow>('SELECT * FROM permission_grants WHERE id = ?', [id])
  ...
  findByUser(userId: string): PermissionGrant[] {
    const results = this.connection.query<PermissionGrantRow>('SELECT * FROM permission_grants WHERE user_id = ?', [
      userId,
    ])
