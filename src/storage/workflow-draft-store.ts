...
class WorkflowDraftStoreImpl implements WorkflowDraftStore {
  ...
  getDraftById(draftId: string): WorkflowDraft | null {
    const results = this.connection.query<{
      ...
      created_at: string
      updated_at: string
    }>(`SELECT * FROM workflow_drafts WHERE draft_id = ?`, [draftId])
  ...
  getDraftsByOwner(ownerUserId: string): WorkflowDraft[] {
    const results = this.connection.query<{
      ...
      created_at: string
      updated_at: string
    }>(`SELECT * FROM workflow_drafts WHERE owner_user_id = ? ORDER BY updated_at DESC`, [ownerUserId])
  ...
  getDraftsByStatus(status: WorkflowDraftStatus): WorkflowDraft[] {
    const results = this.connection.query<{
      ...
      created_at: string
      updated_at: string
    }>(`SELECT * FROM workflow_drafts WHERE status = ? ORDER BY updated_at DESC`, [status])
  ...
  updateDraft(
    ...
    this.connection.exec(`UPDATE workflow_drafts SET ${updateFields.join(', ')} WHERE draft_id = ?`, values)
  ...
  deleteDraft(draftId: string): boolean {
    ...
    this.connection.exec(`DELETE FROM workflow_drafts WHERE draft_id = ?`, [draftId])
