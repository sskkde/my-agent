 ...
 */
export function checkProductionConfig(env: Record<string, string | undefined> = process.env): ProductionGuardResult {
  ...
  if (!secretKey) {
    ...
    errors.push(`APP_SECRET_KEY must be at least 32 characters (current: ${secretKey.length})`)
  } else if (isPlaceholder(secretKey)) {
    errors.push('APP_SECRET_KEY appears to be a placeholder value; set a strong, unique secret')
  }

  const hasAuthToken = !!env.API_AUTH_TOKEN
  const hasApiKeyBootstrap = !!env.API_KEY_BOOTSTRAP
  if (!hasAuthToken && !hasApiKeyBootstrap) {
    errors.push('At least one authentication method must be enabled: set API_AUTH_TOKEN or configure API Key bootstrap')
  }
  ...
  if (!allowedOrigins) {
    ...
    errors.push('ALLOWED_ORIGINS must not be "*" in production; specify explicit comma-separated URLs')
  }

  if (!env.DATABASE_URL && !env.DATABASE_PATH) {
    errors.push('Either DATABASE_URL or DATABASE_PATH must be set in production')
  }

  if (env.LOG_LEVEL?.toLowerCase() === 'debug') {
    errors.push('LOG_LEVEL must not be "debug" in production; use "info", "warn", or "error"')
  }
  ...
  if (!publicBaseUrl) {
    ...
    errors.push(`PUBLIC_BASE_URL must be a valid HTTP or HTTPS URL (current: "${publicBaseUrl}")`)
  }
