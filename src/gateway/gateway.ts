...
export function createGateway(options: GatewayOptions): Gateway {
  ...
  return {
    receiveUserMessage(userId: string, sessionId: string, text: string, channel = 'default', attachmentIds?: string[]): InboundEnvelope {
      ...
      emitGatewayEvent('gateway.inbound_received', userId, sessionId, envelope.envelopeId, {
    ...
    normalizeInbound(rawPayload): InboundEnvelope {
      ...
      emitGatewayEvent('gateway.inbound_received', rawPayload.userId, rawPayload.sessionId, envelope.envelopeId, {
        envelopeType: rawPayload.eventType,
        sourceChannel: rawPayload.sourceChannel,
      })
    ...
    assembleHydratedState(userId: string, sessionId: string, stores: Stores): HydratedSessionState {
      ...
      const lastActivityAt =
        transcripts.length > 0 ? transcripts[transcripts.length - 1].createdAt : new Date().toISOString()
      ...
      const events = stores.eventStore.query({ sessionId }) as Array<{
        eventType: string
        relatedRefs?: { plannerRunId?: string; backgroundRunId?: string }
      }>
    ...
    formatOutbound(responseType, content, recipient, correlationId): OutboundEnvelope {
      ...
      emitGatewayEvent('gateway.outbound_sent', recipient.userId, recipient.sessionId, correlationId, {
        messageType: responseType,
      })
