...
export class ContextManager {
  ...
  private initializePipeline(input: ContextAssemblyInput): PipelineContext {
    ...
    if (input.conversationState?.recentTurns) {
      items.push(
        ...input.conversationState.recentTurns.map((turn, idx) =>
          this.createTurnItem(turn, idx)
        )
      );
    }
    ...
    if (input.workingContext?.recentToolResults) {
      items.push(
        ...input.workingContext.recentToolResults.map((result, idx) =>
          this.createToolResultItem(result, idx)
        )
      );
    }
    ...
    if (input.workingContext?.recentSubagentResults) {
      items.push(
        ...input.workingContext.recentSubagentResults.map((result, idx) =>
          this.createSubagentResultItem(result, idx)
        )
      );
    }
    ...
    for (const item of ctx.scoredItems) {
      ...
      if (item.pairId && item.requiresPairIntegrity) {
        if (!pairIdsSelected.has(item.pairId)) {
          ...
          if (pairGroup) {
            const pairTokens = pairGroup.reduce(
            ...
            const pairItems = pairGroup.filter(
              p => !selected.includes(p) && p.itemId !== item.itemId
            );
            selected.push(...pairItems);
            usedTokens += pairItems.reduce(
              (sum, p) => sum + (p.estimatedTokens || 0),
              0
            );
          }
    ...
    const candidateItemIds = ctx.scoredItems
      .filter(
        item =>
          !item.isPinned &&
          item.isCompressible &&
          !ctx.selectedItems.some(si => si.itemId === item.itemId)
      )
      .slice(0, 10)
