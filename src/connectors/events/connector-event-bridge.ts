...
export class ConnectorEventBridge {
  ...
  private operationIndex: Map<string, {
  ...
  trackAsyncOperation(
    ...
    const timeoutAt = targetInfo.timeoutMs
      ? new Date(now.getTime() + targetInfo.timeoutMs).toISOString()
      : undefined;
  ...
  handleConnectorEvent(event: ConnectorAsyncEvent): {
    ...
    if (newStatus === 'satisfied') {
      ...
      updatedCondition = this.config.waitConditionStore.markCancelled(
        waitConditionId,
        'Connector operation cancelled'
      );
    }

    const triggerEvent = this.emitEvent({
      eventType: newStatus === 'satisfied' 
  ...
  handleCancellation(operationId: string, reason?: string): {
    waitCondition: WaitCondition | null;
    action: RuntimeAction | null;
    syntheticResult?: SyntheticAsyncResult;
  } {
