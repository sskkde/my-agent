...
export class McpToolBridge {
  ...
  async discoverTools(sessionId: string): Promise<ToolDefinition[]> {
    ...
    return descriptors.map(descriptor => this.descriptorToToolDefinition(session.sessionId, session.serverId, descriptor));
  }
  ...
  async registerTools(registry: ToolRegistry, sessionId: string): Promise<void> {
    ...
    this.registeredBySession.set(sessionId, tools.map(tool => tool.name));
  }
  ...
  async callTool(
    ...
    const response = await this.executeWithGuards(transport, rawName, params, requestId, connectorInstanceId, options.signal);
    const normalized = normalizeConnectorResponse(response);
    this.auditToolCall(toolName, params, normalized, options, requestId);
    return normalized;
  }

  private descriptorToToolDefinition(sessionId: string, serverId: string, descriptor: MCPToolDescriptor): ToolDefinition {
  ...
  private async executeWithGuards(
    ...
    try {
      ...
      const raw = await Promise.race([call.then(value => this.rawResultToResponse(value, requestId, connectorInstanceId)), timeout, cancellation]);
      return raw;
    } catch (error) {
      return {
        status: 'failed',
