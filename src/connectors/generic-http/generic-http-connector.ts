...
import type { GenericHttpConfig, RequestTemplate, GenericHttpAuth } from './generic-http-types.js'
...
function resolveConfig(instance: ConnectorInstance): GenericHttpConfig {
  ...
  if (config.openApiImport?.specObject) {
    ...
    const merged: GenericHttpConfig = {
      ...parsed.config,
      ...config,
      requestTemplates: [...(config.requestTemplates ?? []), ...parsed.config.requestTemplates],
    }
...
export class GenericHttpConnectorAdapter implements ConnectorAdapter {
  async execute(instance: ConnectorInstance, request: ConnectorCallRequest): Promise<unknown> {
    ...
    const resolvedHeaders = template.headers ? this.resolveHeaders(template.headers, request.params) : undefined
  ...
  private resolveHeaders(headers: Record<string, string>, params: Record<string, unknown>): Record<string, string> {
