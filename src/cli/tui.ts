      ...
      if (Array.isArray(models) && models.length > 0) {
        for (const model of models) {
          const current = model.current
            ? ` ${COLORS.yellow}[current]${COLORS.reset}`
            : '';
...
async function executeModel(args: string[]): Promise<string> {
  ...
  return `Switched to model: ${COLORS.cyan}${modelName}${COLORS.reset}\n\n` +
...
async function handleProviderConnect(args: string[]): Promise<string> {
  ...
  if (!isValidProviderType(providerType)) {
    return `Error: Invalid provider type "${providerType}"\n\n` +
      `Valid provider types: ${VALID_PROVIDER_TYPES.join(', ')}`;
  }
  ...
  try {
    ...
    if (response.ok) {
      ...
      return `${COLORS.green}✓${COLORS.reset} Connected to ${providerType}\n\n` +
...
async function handleProviderEnable(args: string[], enableValue: boolean): Promise<string> {
  ...
  try {
    ...
    if (response.ok) {
      ...
      const provider = result.data;
      const status = provider.enabled ? `${COLORS.green}enabled${COLORS.reset}` : `${COLORS.gray}disabled${COLORS.reset}`;
      return `${COLORS.green}✓${COLORS.reset} Provider ${action}d\n\n` +
...
async function handleProviderSubcommand(args: string[]): Promise<string> {
  if (args.length === 0) {
    return `Usage: /provider <subcommand>\n\n` +
  ...
  switch (subcommand) {
    ...
    default:
      return `Unknown provider subcommand: ${subcommand}\n\n` +
...
async function executeCommand(parsed: ParsedCommand): Promise<string> {
  ...
  switch (commandName) {
    ...
    default: {
      ...
      const cmd = COMMAND_CATALOG[commandName as keyof typeof COMMAND_CATALOG];
      if (cmd) {
        return `Command /${commandName} is not yet implemented in the TUI.\n` +
