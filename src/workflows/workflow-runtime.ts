...
class WorkflowRuntimeImpl implements WorkflowRuntime {
  ...
  private evaluatePollingCondition(
    ...
    if (result.conditionMet) {
      if (this.waitConditionStore) {
        this.waitConditionStore.markSatisfied(waitCondition.id, 'polling_evaluator', {
          conditionMet: true,
          pollAttempt,
        })
      }
  ...
  private executeConditionStep(stepRunId: string): void {
    ...
    if (!nextStepId) {
      this.workflowRunStore.saveWorkflowOutput(stepRun.workflowRunId, {
        finalOutput: { conditionMet: result.conditionMet },
      })
  ...
  private executeBranchInternalStep(
    ...
    if (this.dispatcher) {
      this.dispatcher
        ...
        .then((result) => {
          this.handleBranchInternalStepCompletion(stepRunId, branchStep, branchSteps, parentBranchStep, {
            success: result.success,
            output: result.result,
            error: result.error,
          })
  ...
  private async executeParallelGroupStep(stepRunId: string): Promise<void> {
    ...
    this.workflowRunStore.updateCurrentSteps(
      stepRun.workflowRunId,
      parallelSteps.map((s) => s.stepId),
    )
