...
import { WEB_FETCH_MAX_RESPONSE_BYTES, WEB_FETCH_MAX_RETURNED_CHARS } from '../../../src/tools/builtins/web-safety.js'
...
describe('web_fetch tool', () => {
  ...
  const mockFetch = (response: { status?: number; body?: string; headers?: Headers }) => {
    ...
    global.fetch = vi.fn().mockResolvedValue({
      ...
      body: {
        getReader: () => ({
          read: vi.fn().mockResolvedValueOnce({ done: false, value: bytes }).mockResolvedValueOnce({ done: true }),
        }),
      },
