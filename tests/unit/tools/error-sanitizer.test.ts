...
describe('sanitizeErrorMessage', () => {
  describe('secret redaction', () => {
    ...
    it('should redact Bearer tokens', () => {
      const message = 'Authorization failed: bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJzdWIiOiIxMjM0NTY3ODkwIn0.dozjgNryP4J3jVmNHl0w5N_XgL0n3I9PlFUP0THsR8U';
      const sanitized = sanitizeErrorMessage(message);
    ...
    it('should redact access tokens', () => {
      const message = 'access_token=ya29.a0AfH6SMBxXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX';
      const sanitized = sanitizeErrorMessage(message);
    ...
    it('should redact long base64-encoded values', () => {
      const message = 'Encoded secret: SGVsbG8gV29ybGQhVGhpcyBpcyBhIHZlcnkgbG9uZyBzZWNyZXQgdGhhdCBzaG91bGQgYmUgcmVkYWN0ZWQ=';
      const sanitized = sanitizeErrorMessage(message);
      expect(sanitized).toContain('[REDACTED]');
    });

    it('should redact private key blocks', () => {
      const message = 'Key error: -----BEGIN PRIVATE KEY-----MIIEvgIBADANBgkqhkiG9w0BAQEFAASCBKgwggSkAgEAAoIBAQC-----END PRIVATE KEY-----';
      const sanitized = sanitizeErrorMessage(message);
  ...
  describe('multiple secrets in one message', () => {
    it('should redact multiple different secret types', () => {
      const message = 'Failed with api_key=sk-test12345678901234567890 and password=secret123 and bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiIxMjM0In0.test';
      const sanitized = sanitizeErrorMessage(message);
...
describe('formatPersistedError', () => {
  ...
  it('should handle different error codes', () => {
    expect(formatPersistedError('SCHEMA_VALIDATION_FAILED', 'Invalid field')).toBe('[SCHEMA_VALIDATION_FAILED] Invalid field');
