...
describe('kernel-guard-constants', () => {
  ...
  describe('mapKernelErrorToForegroundResult', () => {
    ...
    it('LLM error safe failure tested', () => {
      ...
      expect(result.status).toBe('failed')
      expect(result.finalResponse).toBe('The AI service encountered an issue. Please try again.')
    ...
    it('Generic error safe failure tested', () => {
      ...
      expect(result.status).toBe('failed')
      expect(result.finalResponse).toBe('Something went wrong while processing your request. Please try again.')
    ...
    it('does not expose tool params in runtimeSummary', () => {
      ...
      expect(result.runtimeSummary?.toolCallSummaries?.[0]?.summary).toBe('Tool execution interrupted')
    })
  })
