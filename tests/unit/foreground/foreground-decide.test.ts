...
describe('validateForegroundDecideParams', () => {
  ...
  describe('invalid route rejected', () => {
    it('should reject unknown route string', () => {
      const result = validateForegroundDecideParams(validParams({ route: 'invalid_route' as any }), defaultOptions())
      expect(result.valid).toBe(false)
      expect(result.error?.code).toBe('INVALID_ROUTE')
    })

    it('should reject numeric route', () => {
      const result = validateForegroundDecideParams({ ...validParams(), route: 42 as any }, defaultOptions())
      expect(result.valid).toBe(false)
      expect(result.error?.code).toBe('INVALID_ROUTE')
    })

    it('should reject null route', () => {
      const result = validateForegroundDecideParams({ ...validParams(), route: null as any }, defaultOptions())
      expect(result.valid).toBe(false)
      expect(result.error?.code).toBe('INVALID_ROUTE')
    })
  })
  ...
  describe('empty reason rejected', () => {
    ...
    it('should reject non-string reason', () => {
      const result = validateForegroundDecideParams({ ...validParams(), reason: 123 as any }, defaultOptions())
      expect(result.valid).toBe(false)
      expect(result.error?.code).toBe('EMPTY_REASON')
    })

    it('should reject reason exceeding 1000 characters', () => {
      const result = validateForegroundDecideParams(validParams({ reason: 'x'.repeat(1001) }), defaultOptions())
      expect(result.valid).toBe(false)
      expect(result.error?.code).toBe('EMPTY_REASON')
    })
  })
  ...
  describe('schema version validation', () => {
    ...
    it('should reject wrong schemaVersion', () => {
      const result = validateForegroundDecideParams(validParams({ schemaVersion: '2.0' }), defaultOptions())
      expect(result.valid).toBe(false)
      expect(result.error?.code).toBe('INVALID_SCHEMA_VERSION')
    })
  })
...
describe('createForegroundDecideTool', () => {
  ...
  it('should reject empty reason in handler', () => {
    const tool = createForegroundDecideTool()
    const result = tool.handler(validParams({ reason: '' }), mockContext) as {
      success: boolean
