...
describe('provider-crypto', () => {
  ...
  describe('getEncryptionKey', () => {
    ...
    it('should throw with descriptive message when key is missing', () => {
      ...
      expect(() => getEncryptionKey()).toThrow(
        'APP_SECRET_KEY environment variable is required for encrypting provider API keys',
      )
    })
  })
  ...
  describe('decryptSecret', () => {
    ...
    it('should throw DecryptionError when authTag is wrong', () => {
      ...
      expect(() => decryptSecret(encrypted.encrypted, encrypted.iv, wrongAuthTag)).toThrow(DecryptionError)
    })
    ...
    it('should throw DecryptionError when IV is wrong', () => {
      ...
      expect(() => decryptSecret(encrypted.encrypted, wrongIv, encrypted.authTag)).toThrow(DecryptionError)
    })
    ...
    it('should throw when APP_SECRET_KEY is missing', () => {
      ...
      expect(() => decryptSecret(encrypted.encrypted, encrypted.iv, encrypted.authTag)).toThrow(
        MissingEncryptionKeyError,
      )
    })
  })
  ...
  describe('integration: encrypt/decrypt round-trip', () => {
    ...
    it('should produce same decrypted value with different serializations', () => {
      ...
      const decrypted1 = decryptSecret(encrypted1.encrypted, encrypted1.iv, encrypted1.authTag)
      const decrypted2 = decryptSecret(encrypted2.encrypted, encrypted2.iv, encrypted2.authTag)
