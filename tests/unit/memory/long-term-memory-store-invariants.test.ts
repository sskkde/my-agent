...
describe('Long-term Memory Store Invariants', () => {
  ...
  describe('Tombstones', () => {
    it('should create tombstone', () => {
      ...
      const hasTombstone = memoryStore.hasTombstone('user-123', 'fp-deleted', 'hash-window-001')

      expect(hasTombstone).toBe(true)
    })

    it('should return false for non-existent tombstone', () => {
      const hasTombstone = memoryStore.hasTombstone('user-123', 'non-existent', 'hash-window-001')
    ...
    it('should create tombstone on delete', () => {
      ...
      const hasTombstone = memoryStore.hasTombstone('user-123', 'fp-abc123', 'hash-window-001')
      expect(hasTombstone).toBe(true)
    })
