...
describe('Long-term Memory Lifecycle', () => {
  ...
  describe('Write', () => {
    ...
    it('should store memory with entities', () => {
      const memoryWithEntities: LongTermMemoryRecord = {
        ...
        entities: [{ entityType: 'project', displayName: 'My Project' }],
      }
  ...
  describe('Delete', () => {
    ...
    it('should throw error when deleting non-existent memory', () => {
      expect(() => store.delete('non-existent')).toThrow('Memory with id "non-existent" not found')
    })
  ...
  describe('Patch', () => {
    ...
    it('should throw error when patching non-existent memory', () => {
      expect(() => store.applyPatch('non-existent', { importance: 'low' })).toThrow(
