...
describe('computeInstructionHash', () => {
  ...
  describe('content sensitivity', () => {
    it('produces different hashes for different content', () => {
      const blocks1: InstructionBlock[] = [{ source: 'system_prompt', content: 'Hello', priority: 10 }]
      const blocks2: InstructionBlock[] = [{ source: 'system_prompt', content: 'World', priority: 10 }]
    ...
    it('produces different hashes for different sources with same content', () => {
      const blocks1: InstructionBlock[] = [{ source: 'system_prompt', content: 'Same content', priority: 10 }]
      const blocks2: InstructionBlock[] = [{ source: 'routing_prompt', content: 'Same content', priority: 10 }]
