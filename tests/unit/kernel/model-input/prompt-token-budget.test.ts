...
describe('Prompt Token Budget Tests', () => {
    ...
    it('memory policy ≤ 480 chars for typical policy', () => {
      const policy: MemoryPolicyProjection = {
        ...
        priorityRules: [
          ...
          'Critical importance > low importance',
        ],
        invisibilityRules: [
          'Hidden memories should not be referenced',
        ],
        tokenBudget: 500,
      };
  ...
  describe('total projection budget', () => {
    it('total projection increase ≤ 2000 chars with all projections', () => {
      ...
      const totalLength =
        personaRendered.length +
        toolPolicyRendered.length +
        memoryPolicyRendered.length +
        summaryRendered.length;
