...
import type { ModelInputBuildInput, MemoryPolicyProjection } from '../../../../src/kernel/model-input/model-input-types.js';
...
describe('MemoryPolicyProjection', () => {
  describe('renderMemoryPolicyProjection', () => {
    ...
    it('token budget constraint: rendered text ≤ 480 characters (120 tokens estimate)', () => {
      const policy: MemoryPolicyProjection = {
        useRules: 'Memory is private background context. Default is not to say "I remember" or "according to my memory". Memory is only used to enhance continuity and assumption calibration. Current conversation takes priority over old memories. No over-evidence inference. Only cite carefully when user explicitly asks.',
