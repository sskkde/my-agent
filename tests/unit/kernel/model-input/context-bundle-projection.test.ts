...
import { projectContextBundle, type DynamicFields } from '../../../../src/kernel/model-input/context-bundle-projection.js';
...
describe('projectContextBundle', () => {
  describe('full ContextBundle projection', () => {
    it('projects all fields with correct roles and order', () => {
      const bundle: ContextBundleData = {
        pinnedItems: [
          { itemId: 'p1', content: 'Pinned instruction', semanticType: 'instruction' },
        ],
        orderedItems: [
          { itemId: 'o1', content: 'Some fact', semanticType: 'fact' },
        ],
        ...
        summaryBlocks: [
          { itemId: 's1', content: 'Summary of prior turns', semanticType: 'summary' },
        ],
        transcript: [
  ...
  describe('semanticType role mapping in projection', () => {
    it('constraint items map to system', () => {
      const bundle: ContextBundleData = {
        orderedItems: [
          { itemId: 'c1', content: 'Must follow rule', semanticType: 'constraint' },
        ],
      };
    ...
    it('draft items map to assistant', () => {
      const bundle: ContextBundleData = {
        orderedItems: [
          { itemId: 'd1', content: 'Draft text', semanticType: 'draft' },
        ],
      };
      const result = projectContextBundle(bundle, {});
    ...
    it('summary items map to assistant', () => {
      const bundle: ContextBundleData = {
        summaryBlocks: [
          { itemId: 's1', content: 'Summary text', semanticType: 'summary' },
        ],
      };
      const result = projectContextBundle(bundle, {});
      expect(result.messages[0].role).toBe('assistant');
    ...
    it('plan_view items map to system', () => {
      const bundle: ContextBundleData = {
        orderedItems: [
          { itemId: 'p1', content: 'Plan step', semanticType: 'plan_view' },
        ],
      };
      const result = projectContextBundle(bundle, {});
      expect(result.messages[0].role).toBe('system');
  ...
  describe('optional field ordering is deterministic', () => {
    it('always produces planView → workflowStepView → backgroundRunView → triggerView → summaryBlocks', () => {
      const bundle: ContextBundleData = {
        ...
        summaryBlocks: [
          { itemId: 's1', content: 'Summary at end', semanticType: 'summary' },
        ],
  ...
  describe('currentUserMessage positioning', () => {
    it('currentUserMessage is ALWAYS the last message', () => {
      const bundle: ContextBundleData = {
        pinnedItems: [
          { itemId: 'p1', content: 'Pinned', semanticType: 'instruction' },
        ],
        summaryBlocks: [
          { itemId: 's1', content: 'Summary', semanticType: 'summary' },
        ],
        transcript: [
