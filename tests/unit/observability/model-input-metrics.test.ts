...
describe('ModelInputMetrics', () => {
  ...
  describe('getAggregateByAgent', () => {
    it('aggregates metrics by agent kind', () => {
      ...
      const foregroundAgg = aggregates.find(
        (a: { agentKind: string; count: number; avgCacheHitRate: number; totalTokens: number }) =>
          a.agentKind === 'foreground',
      )
      ...
      const searchAgg = aggregates.find(
        (a: { agentKind: string; count: number; avgCacheHitRate: number; totalTokens: number }) =>
          a.agentKind === 'search',
      )
