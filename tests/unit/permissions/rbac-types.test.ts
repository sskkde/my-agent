...
describe('RBAC Types', () => {
  ...
  describe('ROLE_PERMISSIONS mapping', () => {
    ...
    it('admin should have ALL ResourceType x Action permissions', () => {
      ...
      allResourceTypes.forEach((resource) => {
        allActions.forEach((action) => {
          expect(adminPermissions.some((p) => p.resource === resource && p.action === action)).toBe(true)
        })
      })
    })
    ...
    it('user should have CRUD on own resources', () => {
      ...
      ownResourceTypes.forEach((resource) => {
        crudActions.forEach((action) => {
          expect(userPermissions.some((p) => p.resource === resource && p.action === action)).toBe(true)
        })
      })
    })
    ...
    it('user should have read on public resources', () => {
      const userPermissions = ROLE_PERMISSIONS.user
      const publicResources = [ResourceType.connectors, ResourceType.observability]

      publicResources.forEach((resource) => {
        expect(userPermissions.some((p) => p.resource === resource && p.action === Action.read)).toBe(true)
      })
    })
    ...
    it('user should NOT have manage or execute on most resources', () => {
      ...
      expect(userPermissions.some((p) => p.resource === ResourceType.sessions && p.action === Action.manage)).toBe(
        false,
      )

      // User should not have execute on connectors
      expect(userPermissions.some((p) => p.resource === ResourceType.connectors && p.action === Action.execute)).toBe(
        false,
      )
    })
    ...
    it('service should only have execute on authorized API endpoints', () => {
      ...
      const executeResources = [ResourceType.workflows, ResourceType.triggers, ResourceType.connectors]

      executeResources.forEach((resource) => {
        expect(servicePermissions.some((p) => p.resource === resource && p.action === Action.execute)).toBe(true)
      })

      // Service should NOT have other actions
      expect(servicePermissions.some((p) => p.action === Action.create)).toBe(false)
      expect(servicePermissions.some((p) => p.action === Action.update)).toBe(false)
      expect(servicePermissions.some((p) => p.action === Action.delete)).toBe(false)
      expect(servicePermissions.some((p) => p.action === Action.manage)).toBe(false)
    })
  })

  describe('hasPermission helper', () => {
    it('should return true when role has the permission', () => {
      expect(hasPermission('admin', ResourceType.sessions, Action.create)).toBe(true)
      expect(hasPermission('user', ResourceType.sessions, Action.read)).toBe(true)
      expect(hasPermission('service', ResourceType.workflows, Action.execute)).toBe(true)
    })

    it('should return false when role lacks the permission', () => {
      expect(hasPermission('user', ResourceType.users, Action.delete)).toBe(false)
      expect(hasPermission('service', ResourceType.sessions, Action.create)).toBe(false)
    })
  })
