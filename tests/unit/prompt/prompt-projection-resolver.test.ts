...
describe('PromptProjectionResolver', () => {
  ...
  describe('template loading failure', () => {
    it('should fallback to defaults and warn when template not registered', async () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Template not registered: heuristics:tool-usage.common'));
    ...
    it('should fallback to defaults and warn when loader throws', async () => {
      ...
      expect(warnSpy).toHaveBeenCalledWith(expect.stringContaining('Failed to load template heuristics:tool-usage.common'));
