...
function createSuccessfulKernelResult(
  overrides: Partial<KernelRunResult> & { finalResponse: string },
): KernelRunResult {
  return {
...
describe('Kernel-Driven Foreground Turn Integration Tests', () => {
      ...
      const kernelResult = createSuccessfulKernelResult({
        ...
        transcript: [
          ...
          },
          {
            ...
            type: 'tool_result',
            content: { toolCallId: 'tc-search-001', result: searchResult },
          },
          {
            iteration: 1,
            timestamp: '2024-01-15T10:00:03.000Z',
            type: 'llm_response',
            content: { content: 'The weather in Tokyo is currently sunny with a temperature of 22°C.' },
          },
        ],
      })
  ...
  describe('Complex task planner creation', () => {
    it('calls foreground_spawn_planner and returns plannerRunId in runtimeSummary', async () => {
      const kernelResult = createSuccessfulKernelResult({
        finalResponse:
          "I've created a plan for your 3-day Tokyo trip. The planner will organize hotel bookings, sightseeing, and restaurant reservations.",
        toolCalls: [
          {
            toolCallId: 'tc-planner-001',
            toolName: 'foreground_spawn_planner',
            params: { objective: 'Plan a 3-day trip to Tokyo' },
          },
        ],
        transcript: [
          {
            ...
            timestamp: '2024-01-15T10:00:01.000Z',
            type: 'tool_call',
            content: { toolCallId: 'tc-planner-001', toolName: 'foreground_spawn_planner' },
          },
          {
            iteration: 1,
            timestamp: '2024-01-15T10:00:02.000Z',
            type: 'tool_result',
            content: { toolCallId: 'tc-planner-001', result: { plannerRunId: 'planner-run-001', status: 'created' } },
          },
          {
            iteration: 1,
            timestamp: '2024-01-15T10:00:03.000Z',
            type: 'llm_response',
            content: { content: "I've created a plan for your 3-day Tokyo trip." },
          },
        ],
      })
  ...
  describe('Status query', () => {
    it('calls foreground_status_query and returns current status', async () => {
      const kernelResult = createSuccessfulKernelResult({
        ...
        toolCalls: [{ toolCallId: 'tc-status-001', toolName: 'foreground_status_query', params: {} }],
        transcript: [
          {
            iteration: 1,
            timestamp: '2024-01-15T10:00:01.000Z',
            type: 'tool_call',
            content: { toolCallId: 'tc-status-001', toolName: 'foreground_status_query' },
          },
          {
            iteration: 1,
            timestamp: '2024-01-15T10:00:02.000Z',
            type: 'tool_result',
            content: {
              toolCallId: 'tc-status-001',
              result: { activeTasks: 2, details: ['Tokyo trip planner', 'Quantum computing research'] },
            },
          },
          {
            iteration: 1,
            timestamp: '2024-01-15T10:00:03.000Z',
            type: 'llm_response',
            content: { content: 'You have 2 active tasks.' },
          },
        ],
      })
  ...
  describe('Subagent launch', () => {
    it('calls foreground_launch_subagent and returns runtimeActionId', async () => {
      const kernelResult = createSuccessfulKernelResult({
        finalResponse:
          "I've launched a background research agent to investigate quantum computing advances. It will report back when complete.",
        toolCalls: [
          {
            toolCallId: 'tc-subagent-001',
            toolName: 'foreground_launch_subagent',
            params: { taskSpec: { objective: 'Research quantum computing' } },
          },
        ],
        transcript: [
          {
            ...
            timestamp: '2024-01-15T10:00:01.000Z',
            type: 'tool_call',
            content: { toolCallId: 'tc-subagent-001', toolName: 'foreground_launch_subagent' },
          },
          {
            iteration: 1,
            timestamp: '2024-01-15T10:00:02.000Z',
            ...
            content: {
              toolCallId: 'tc-subagent-001',
              result: { runtimeActionId: 'action-sub-001', agentType: 'research', status: 'launched' },
            },
          },
          {
            iteration: 1,
            timestamp: '2024-01-15T10:00:03.000Z',
            type: 'llm_response',
            content: { content: "I've launched a background research agent." },
          },
        ],
      })
  ...
  describe('Tool failure', () => {
    it('returns safe user-facing response when a tool fails', async () => {
      const kernelResult = createSuccessfulKernelResult({
        finalResponse:
          "I'm sorry, but I wasn't able to search the web right now. The search service appears to be temporarily unavailable. Please try again in a moment.",
        toolCalls: [{ toolCallId: 'tc-search-fail-001', toolName: 'search_subagent', params: { query: 'test query' } }],
        transcript: [
          {
            iteration: 1,
            timestamp: '2024-01-15T10:00:01.000Z',
            type: 'tool_call',
            content: { toolCallId: 'tc-search-fail-001', toolName: 'search_subagent' },
          },
          {
            iteration: 1,
            timestamp: '2024-01-15T10:00:02.000Z',
          ...
          },
          {
            ...
            timestamp: '2024-01-15T10:00:03.000Z',
            type: 'llm_response',
            content: { content: "I'm sorry, but I wasn't able to search the web right now." },
          },
        ],
      })
    ...
    it('returns user-safe error when kernel itself fails', async () => {
      const failedKernelResult: KernelRunResult = {
        finalStatus: 'failed',
        iterationsUsed: 1,
        toolCalls: [{ toolCallId: 'tc-fail-001', toolName: 'web_search', params: { query: 'test' } }],
        transcript: [],
  ...
  describe('Unprojected tool rejection', () => {
    it('kernel rejects unprojected tool call with UNPROJECTED_TOOL_CALL error', async () => {
      ...
      const kernelResult = createSuccessfulKernelResult({
        ...
        toolCalls: [{ toolCallId: 'tc-unproj-001', toolName: 'admin_delete_all', params: {} }],
        transcript: [
          {
            iteration: 1,
            timestamp: '2024-01-15T10:00:01.000Z',
            type: 'tool_call',
            content: { toolCallId: 'tc-unproj-001', toolName: 'admin_delete_all' },
          },
          {
            iteration: 1,
            timestamp: '2024-01-15T10:00:02.000Z',
          ...
          },
          {
            ...
            timestamp: '2024-01-15T10:00:03.000Z',
            type: 'llm_response',
            content: { content: "I can't perform that action." },
          },
        ],
      })
  ...
  describe('ProcessorOrchestration routing', () => {
    ...
    it('propagates runtimeSummary from kernel result to output', async () => {
      const kernelResult = createSuccessfulKernelResult({
        ...
        toolCalls: [{ toolCallId: 'tc-001', toolName: 'search_subagent', params: { query: 'test' } }],
        transcript: [
          { iteration: 1, timestamp: '2024-01-15T10:00:01.000Z', type: 'tool_call', content: { toolCallId: 'tc-001' } },
          {
            ...
            timestamp: '2024-01-15T10:00:02.000Z',
            type: 'tool_result',
            content: { toolCallId: 'tc-001', result: 'data' },
          },
        ],
      })
      ...
      const runtimeSummary = output.result?.data?.runtimeSummary as {
        toolCallSummaries?: Array<{ toolCallId: string; toolName: string }>
      }
      expect(runtimeSummary?.toolCallSummaries).toBeDefined()
