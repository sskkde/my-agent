...
describe('Observability Console API', () => {
  ...
  describe('GET /api/observability/runs/:runId/replay-preview', () => {
    it('should return replay preview for planner run', async () => {
      const response = await fetch(`${baseUrl}/api/v1/observability/runs/${plannerRunId}/replay-preview`, {
        headers: { Cookie: authCookie },
      })
      expect(response.status).toBe(200)
    ...
    it('should return replay preview for workflow run', async () => {
      const response = await fetch(`${baseUrl}/api/v1/observability/runs/${workflowRunId}/replay-preview`, {
        headers: { Cookie: authCookie },
      })
      expect(response.status).toBe(200)
    ...
    it('should return 404 for unknown runId', async () => {
      const response = await fetch(`${baseUrl}/api/v1/observability/runs/unknown-run/replay-preview`, {
        headers: { Cookie: authCookie },
      })
      expect(response.status).toBe(404)
