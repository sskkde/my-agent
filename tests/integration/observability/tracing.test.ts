...
describe('Observability Tracing Integration', () => {
  ...
  describe('RuntimeSpan parent/child linkage', () => {
    it('should create a span with parent reference', () => {
      ...
      const parentSpan = collector.startSpan(trace.traceId, 'kernel_run', 'kernel', 'execute_agent', trace.rootSpanId)

      const childSpan = collector.startSpan(trace.traceId, 'tool_execution', 'tool', 'call_api', parentSpan.spanId)
    ...
    it('should find child spans by parent ID', () => {
      ...
      collector.startSpan(trace.traceId, 'kernel_run', 'kernel', 'step_1', parentSpan.spanId)

      collector.startSpan(trace.traceId, 'kernel_run', 'kernel', 'step_2', parentSpan.spanId)
  ...
  describe('Metric recording', () => {
    it('should record a metric', () => {
      const trace = collector.startTrace({ userId: 'user_001' })
      const span = collector.startSpan(trace.traceId, 'kernel_run', 'kernel', 'execute', trace.rootSpanId)
  ...
  describe('Tracing hooks', () => {
    it('should create dispatch span with hooks', () => {
      ...
      const span = hooks.onDispatch(trace.traceId, 'kernel_plane', 'run_agent', trace.rootSpanId)
  ...
  describe('Trace lifecycle', () => {
    ...
    it('should end active spans when trace ends', () => {
      ...
      const span1 = collector.startSpan(trace.traceId, 'kernel_run', 'kernel', 'run1', trace.rootSpanId)

      const span2 = collector.startSpan(trace.traceId, 'tool_execution', 'tool', 'tool1', span1.spanId)
