...
async function createMockCollectorServer(
  port: number,
): Promise<{ server: Server; getCapturedRequests: () => CapturedRequest[] }> {
...
describe('OTel Trace Exporter Integration', () => {
  ...
  describe('OTLP JSON format conversion', () => {
    it('should convert RuntimeSpan to OTLP JSON format', async () => {
      ...
      const span = collector.startSpan(trace.traceId, 'kernel_run', 'kernel', 'execute_agent', trace.rootSpanId)
    ...
    it('should include error status for failed spans', async () => {
      const trace = collector.startTrace({ userId: 'user_001' })
      const span = collector.startSpan(trace.traceId, 'tool_execution', 'tool', 'failing_operation', trace.rootSpanId)
    ...
    it('should convert span attributes correctly', async () => {
      const trace = collector.startTrace({ userId: 'user_001' })
      const span = collector.startSpan(trace.traceId, 'kernel_run', 'kernel', 'test_operation', trace.rootSpanId, {
        customAttr: 'value',
        numericAttr: 42,
        boolAttr: true,
      })
