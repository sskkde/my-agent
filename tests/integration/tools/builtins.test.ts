...
class MockSummaryStore implements SummaryStore {
  ...
  getWorkingSummary(runId: string): SummaryRecord | null {
    return (
      Array.from(this.summaries.values()).find((s) => s.runId === runId && s.summaryType === 'working_summary') ?? null
    )
  }

  getSessionMemory(sessionId: string): SummaryRecord | null {
    return (
      Array.from(this.summaries.values()).find(
...
class MockTranscriptStore implements TranscriptStore {
  ...
  search(query: string): TurnTranscript[] {
    return Array.from(this.transcripts.values()).filter(
      (t) =>
        t.input.userMessageSummary?.includes(query) || t.output.visibleMessages.some((m) => m.content.includes(query)),
    )
  }
...
class MockLongTermMemoryStore implements LongTermMemoryStore {
  ...
  getByUserId(userId: string): LongTermMemoryRecord[] {
    return Array.from(this.memories.values()).filter((m) => m.userId === userId && m.lifecycle.status !== 'deleted')
  }
  ...
  getByEntityName(entityName: string, limit?: number): LongTermMemoryRecord[] {
    return Array.from(this.memories.values())
      .filter((m) => m.lifecycle.status !== 'deleted' && m.entityNames?.includes(entityName))
      .slice(0, limit ?? 10)
  }
  ...
  getByDateRange(startDate: string, endDate: string, limit?: number): LongTermMemoryRecord[] {
    return Array.from(this.memories.values())
      .filter(
        (m) =>
          m.lifecycle.status !== 'deleted' && m.lifecycle.createdAt >= startDate && m.lifecycle.createdAt <= endDate,
      )
    ...
    it('should store large results in ToolResultStore and return preview + resultRef', async () => {
      ...
      for (let i = 0; i < 1000; i++) {
        const transcript: TurnTranscript = {
          ...
          output: {
            visibleMessages: [
              { messageId: `msg_${i}`, role: 'assistant', content: `Response ${i} ${'y'.repeat(500)}` },
            ],
          },
          visibility: 'public',
          createdAt: new Date().toISOString(),
