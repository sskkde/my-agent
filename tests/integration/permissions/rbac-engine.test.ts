...
import {
  ...
  getRolePermissions,
  filterResources,
} from '../../../src/permissions/rbac-engine.js';
import {
  Role,
  ResourceType,
  Action,
      ...
      it('should pass ALL permission checks for admin', () => {
        ...
        for (const resource of resources) {
          for (const action of actions) {
            expect(
              checkPermission('admin', resource, action),
              `admin should be allowed ${action} on ${resource}`
            ).toBe(true);
          }
        }
      });
  ...
  describe('hasPermission (from rbac-types)', () => {
    it('should be consistent with checkPermission for basic checks', () => {
      ...
      for (const role of roles) {
        for (const resource of resources) {
          for (const action of actions) {
            expect(hasPermission(role, resource, action)).toBe(
