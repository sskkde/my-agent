...
import { createTranscriptStore, type TranscriptStore, type TurnTranscript, type Visibility } from '../../../src/storage/transcript-store.js';
...
describe('TranscriptStore', () => {
  ...
  function loadTranscriptMigration() {
    ...
    const content = fs.readFileSync(
      path.join(process.cwd(), 'migrations/001_create_transcripts_table.sql'),
      'utf-8'
    );
  ...
  function createTestTranscript(overrides: Partial<TurnTranscript> = {}): TurnTranscript {
    return {
      ...
      output: {
        visibleMessages: [
          { messageId: 'msg-1', role: 'assistant', content: 'Hello! How can I help?' }
        ],
  ...
  describe('findBySession', () => {
    ...
    it('should support limit parameter', () => {
      ...
      for (let i = 0; i < 5; i++) {
        store.saveTurn(createTestTranscript({
          turnId: `turn-${i}`,
          sessionId,
          createdAt: new Date(2026, 0, 1, 10, i).toISOString()
        }));
  ...
  describe('search', () => {
    it('should search transcripts by user-visible content', () => {
      store.saveTurn(createTestTranscript({
        ...
        output: {
          visibleMessages: [
            { messageId: 'm1', role: 'assistant', content: 'Weather is sunny today' }
          ]
        }
      }));
      ...
      store.saveTurn(createTestTranscript({
        ...
        output: {
          visibleMessages: [
            { messageId: 'm2', role: 'assistant', content: 'The time is 3pm' }
          ]
        }
      }));
    ...
    it('should search by user message summary', () => {
      store.saveTurn(createTestTranscript({
        ...
        output: {
          visibleMessages: [
            { messageId: 'm1', role: 'assistant', content: 'I will help you search' }
          ]
        }
      }));
