...
import { createApiKeyStore, type ApiKeyStore } from '../../../src/storage/api-key-store.js'
import { createProviderConfigStore, type ProviderConfigStore } from '../../../src/storage/provider-config-store.js'
import { createUserStore, type UserStore } from '../../../src/storage/user-store.js'
...
describe('Backup/Restore GA Gate', () => {
  ...
  beforeEach(() => {
    ...
    const providerConfigColumns = new Set(
      connection.query<{ name: string }>('PRAGMA table_info(provider_configs)').map((column) => column.name),
    )
  ...
  describe('Backup Creation', () => {
    ...
    it('should verify backup file integrity', async () => {
      ...
      const integrityResult = backupConn.query<{ integrity_check: string }>('PRAGMA integrity_check')
  ...
  describe('Data Consistency After Restore', () => {
    ...
    it('should verify data consistency after restore', async () => {
      ...
      const sessionCountBefore =
        sourceConn.query<{ count: number }>('SELECT COUNT(*) as count FROM sessions')[0]?.count ?? 0

      const userCountBefore = sourceConn.query<{ count: number }>('SELECT COUNT(*) as count FROM users')[0]?.count ?? 0

      const workflowCountBefore =
        sourceConn.query<{ count: number }>('SELECT COUNT(*) as count FROM workflow_runs')[0]?.count ?? 0
      ...
      const sessionCountAfter =
        restoreConn.query<{ count: number }>('SELECT COUNT(*) as count FROM sessions')[0]?.count ?? 0

      const userCountAfter = restoreConn.query<{ count: number }>('SELECT COUNT(*) as count FROM users')[0]?.count ?? 0

      const workflowCountAfter =
        restoreConn.query<{ count: number }>('SELECT COUNT(*) as count FROM workflow_runs')[0]?.count ?? 0
  ...
  describe('Secret Preservation', () => {
    it('should verify API keys are still hashed after restore', async () => {
      ...
      const allKeyData = restoreConn.query<{ key_hash: string }>('SELECT key_hash FROM api_keys WHERE id = ?', [
        'key-001',
      ])
    ...
    it('should verify encrypted auth state is preserved after restore', async () => {
      ...
      testConn.exec('INSERT INTO test_secrets (id, encrypted_value) VALUES (?, ?)', ['secret-001', serialized])

      // Retrieve and decrypt
      const row = testConn.query<{ encrypted_value: string }>('SELECT encrypted_value FROM test_secrets WHERE id = ?', [
        'secret-001',
      ])[0]
  ...
  describe('Error Handling', () => {
    it('should detect corrupt backup file', async () => {
      ...
      try {
        corruptConn.open()
        const result = corruptConn.query<{ integrity_check: string }>('PRAGMA integrity_check')
