...
describe('Orphan Run Recovery', () => {
  ...
  describe('scanOrphanedRuns', () => {
    ...
    it('should handle mixed recoverable and non-recoverable orphans', () => {
      ...
      const plannerStore: OrphanScannerStore = {
        listActive: () => [
          { runId: 'planner-1', runType: 'PlannerRun', status: 'planning', startedAt: longAgo },
        ],
      };

      const kernelStore: OrphanScannerStore = {
        listActive: () => [
          { runId: 'kernel-1', runType: 'KernelRun', status: 'running', startedAt: longAgo },
        ],
      };
      ...
      const recoveredIds = result.orphans
        .filter(o => o.action === 'recover')
        .map(o => o.runId);
      expect(recoveredIds).toEqual(expect.arrayContaining(['planner-1', 'kernel-1']));

      const failedIds = result.orphans
        .filter(o => o.action === 'mark_failed')
        .map(o => o.runId);
