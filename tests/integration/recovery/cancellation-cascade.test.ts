...
import type { CancellationRequest, CancellationCoordinatorConfig, RetryPolicy } from '../../../src/recovery/types.js'
...
describe('Cancellation Cascade and Failure Recovery', () => {
    ...
    it('should handle nested cascade: PlannerRun -> KernelRun -> ToolExecutions', async () => {
      const mockPlannerRunStore = {
        getById: vi.fn().mockReturnValue({
          ...
          status: 'planning',
          checkpoint: {
            activeExecutionRefs: [{ refId: 'kernel_1', refType: 'kernel_run', status: 'running' }],
          },
        }),
        updateStatus: vi.fn(),
      }
    ...
    it('should not cascade when cascade flag is false', async () => {
      const mockPlannerRunStore = {
        getById: vi.fn().mockReturnValue({
          ...
          status: 'planning',
          checkpoint: {
            activeExecutionRefs: [{ refId: 'tool_1', refType: 'tool_execution', status: 'running' }],
          },
        }),
        updateStatus: vi.fn(),
      }
  ...
  describe('Timeout Handling', () => {
    ...
    it('should handle timeout during retry attempt', async () => {
      ...
      const retryExecutor = createRetryExecutor(config)
      const result = await retryExecutor.executeWithRetry({ operation, operationName: 'retryOp' }, retryPolicy)
    ...
    it('should include side effect notice in PlannerRun cancellation result', async () => {
      const mockPlannerRunStore = {
        getById: vi.fn().mockReturnValue({
          ...
          status: 'planning',
          checkpoint: {
            activeExecutionRefs: [{ refId: 'tool_sendEmail', refType: 'tool_execution', status: 'running' }],
          },
        }),
        updateStatus: vi.fn(),
      }
  ...
  describe('Recovery Scenarios', () => {
    it('should not retry non-retryable errors', async () => {
      ...
      const result = await retryExecutor.executeWithRetry({ operation, operationName: 'protectedOp' }, retryPolicy)
    ...
    it('should respect doNotRetryOn list', async () => {
      ...
      const retryExecutor = createRetryExecutor({})
      const result = await retryExecutor.executeWithRetry({ operation, operationName: 'modelOp' }, retryPolicy)
    ...
    it('should use correct backoff delays', async () => {
      ...
      await retryExecutor.executeWithRetry({ operation, operationName: 'backoffTest' }, retryPolicy)
