...
describe('Logs API', () => {
  ...
  describe('GET /api/logs', () => {
    ...
    it('should filter logs by sourceModule', async () => {
      const response = await fetch(`${baseUrl}/api/v1/logs?sessionId=${testSessionId}&sourceModule=tool`, {
        headers: { Cookie: authCookie },
      })
      expect(response.status).toBe(200)
    ...
    it('should filter logs by eventType', async () => {
      const response = await fetch(`${baseUrl}/api/v1/logs?sessionId=${testSessionId}&eventType=run_progress`, {
        headers: { Cookie: authCookie },
      })
      expect(response.status).toBe(200)
    ...
    it('should respect limit parameter', async () => {
      const response = await fetch(`${baseUrl}/api/v1/logs?sessionId=${testSessionId}&limit=2`, {
        headers: { Cookie: authCookie },
      })
      expect(response.status).toBe(200)
    ...
    it('should enforce max limit of 200', async () => {
      const response = await fetch(`${baseUrl}/api/v1/logs?sessionId=${testSessionId}&limit=500`, {
        headers: { Cookie: authCookie },
      })
      expect(response.status).toBe(200)
    ...
    it('should respect offset parameter', async () => {
      const response = await fetch(`${baseUrl}/api/v1/logs?sessionId=${testSessionId}&limit=2&offset=2`, {
        headers: { Cookie: authCookie },
      })
      expect(response.status).toBe(200)
    ...
    it('should return empty array for non-existent session', async () => {
      const response = await fetch(`${baseUrl}/api/v1/logs?sessionId=non-existent-session`, {
        headers: { Cookie: authCookie },
      })
      expect(response.status).toBe(200)
  ...
  describe('GET /api/logs/stream', () => {
    it('should return event stream with text/event-stream content type', async () => {
      ...
      try {
        const response = await fetch(`${baseUrl}/api/v1/logs/stream?sessionId=${testSessionId}`, {
    ...
    it('should filter events after specified eventId', async () => {
      ...
      try {
        const response = await fetch(`${baseUrl}/api/v1/logs/stream?sessionId=${testSessionId}&after=evt-002`, {
          signal: controller.signal,
          headers: { Cookie: authCookie },
        })
        const reader = response.body?.getReader()
