...
describe('Memory Management API', () => {
  ...
  describe('GET /api/memory', () => {
    ...
    it('should filter by memory type', async () => {
      const preferenceMemory = createTestMemory({
        memoryType: 'user_preference',
        content: { text: 'Preference for type filter test' },
      })
      const profileMemory = createTestMemory({
        memoryId: 'mem-profile-type-test',
        memoryType: 'user_profile',
        content: { text: 'Profile for type filter test' },
      })
    ...
    it('should respect limit parameter', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        const memory = createTestMemory({
          memoryId: `mem-limit-test-${Date.now()}-${i}`,
          content: { text: `Limit test memory ${i}` },
        })
