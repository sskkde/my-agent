...
async function readSnapshotFromStream(
  ...
  authCookie: string,
  timeoutMs = 3000,
): Promise<{ snapshotData: SnapshotData; response: Response }> {
  ...
  describe('GET /api/sessions/:sessionId/timeline/stream', () => {
    ...
    it('should receive snapshot with timeline events after seeding data', async () => {
      const turn: TurnTranscript = {
        ...
        output: {
          visibleMessages: [{ messageId: 'msg-stream-001', role: 'assistant', content: 'Test response' }],
        },
        visibility: 'public',
    ...
    it('should skip events with after parameter', async () => {
      ...
      const resultWithoutAfter = await readSnapshotFromStream(
        `${baseUrl}/api/v1/sessions/${newSessionId}/timeline/stream`,
        authCookie,
      )
      ...
      const resultWithAfter = await readSnapshotFromStream(
        `${baseUrl}/api/v1/sessions/${newSessionId}/timeline/stream?after=${firstEventId}`,
        authCookie,
      )
    ...
    it('should return all events when after cursor not found', async () => {
      ...
      const turn: TurnTranscript = {
        ...
        output: {
          visibleMessages: [{ messageId: 'msg-notfound-001', role: 'assistant', content: 'Response' }],
        },
        visibility: 'public',
      ...
      const result = await readSnapshotFromStream(
        `${baseUrl}/api/v1/sessions/${newSessionId}/timeline/stream?after=non-existent-event-id`,
        authCookie,
      )
      expect(result.snapshotData.events).toHaveLength(2)
    })
