...
describe('Provider API: GET /:id and headersConfigured (F3 bugs)', () => {
  ...
  it('GET /api/v1/providers/:id returns 200 with provider data', async () => {
    const create = await server.inject({
      ...
      headers: authed(),
      payload: { providerType: 'openai', displayName: 'F3A', apiKey: 'sk-a', baseUrl: 'https://x', selectedModel: 'g' },
    })
  ...
  it('headersConfigured flips to true after PATCH with non-empty headers', async () => {
    const create = await server.inject({
      ...
      url: '/api/v1/providers',
      headers: authed(),
      payload: { providerType: 'openai', displayName: 'F3B', apiKey: 'sk-b', baseUrl: 'https://x', selectedModel: 'g' },
    })
    const id = JSON.parse(create.body).data.providerId
    await server.inject({
      method: 'PATCH',
  ...
  it('headersConfigured flips to false after PATCH with empty headers', async () => {
    const create = await server.inject({
      ...
      url: '/api/v1/providers',
      headers: authed(),
      payload: { providerType: 'openai', displayName: 'F3C', apiKey: 'sk-c', baseUrl: 'https://x', selectedModel: 'g' },
    })
    const id = JSON.parse(create.body).data.providerId
    await server.inject({
      method: 'PATCH',
