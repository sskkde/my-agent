...
describe('Logs & Debug API - Extended', () => {
  ...
  describe('GET /api/debug/replay/:sessionId - redactedPreviews', () => {
    ...
    it('should redact sensitive fields in low sensitivity previews', async () => {
      ...
      const lowSensitivityPreview = body.data.redactedPreviews.find((p) => p.eventId === 'evt-ld-001')
    ...
    it('should fully redact high sensitivity previews', async () => {
      ...
      const highSensitivityPreview = body.data.redactedPreviews.find((p) => p.eventId === 'evt-ld-002')
    ...
    it('should return all event previews', async () => {
      ...
      const response = await fetch(`${baseUrl}/api/v1/debug/replay/${manyEventsSessionId}`, {
        headers: { Cookie: authCookie },
      })
      const body = (await response.json()) as {
        data: {
  ...
  describe('GET /api/logs - runRef filter', () => {
    it('should filter logs by runRef', async () => {
      const response = await fetch(`${baseUrl}/api/v1/logs?sessionId=${testSessionId}&runRef=run-ld-001`, {
        headers: { Cookie: authCookie },
      })
      expect(response.status).toBe(200)
    ...
    it('should return empty when runRef matches no events', async () => {
      const response = await fetch(`${baseUrl}/api/v1/logs?sessionId=${testSessionId}&runRef=non-existent-run`, {
        headers: { Cookie: authCookie },
      })
      expect(response.status).toBe(200)
