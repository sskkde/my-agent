...
describe('Shadow Extraction Comparison Integration', () => {
  ...
  describe('legacy vs semantic-policy comparison', () => {
    it('should show both accept relevant memory content', () => {
      ...
      const legacyResult = {
        candidates: [
          ...
          makeCandidate({ text: 'Works with TypeScript' }),
        ],
      };
    ...
    it('should compute diff=different when counts match but content differs', () => {
      ...
      const legacyResult = {
        candidates: [
          makeCandidate({ text: 'Preference A' }),
          makeCandidate({ text: 'Preference B' }),
        ],
      };

      const newResult = {
        candidates: [
          makeCandidate({ text: 'Preference X' }),
          makeCandidate({ text: 'Preference Y' }),
        ],
      };
