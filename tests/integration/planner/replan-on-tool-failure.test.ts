...
import {
  createPlannerRunStore,
  type PlannerRunStore,
  type PlannerRunRecord,
} from '../../../src/storage/planner-run-store.js'
...
import { createPlannerRuntime, type PlannerRuntime } from '../../../src/planner/planner-runtime.js'
import type { PlannerRunInput } from '../../../src/planner/types.js'
...
describe('Planner: Tool Failure Replan', () => {
  ...
  it('triggers replan and transitions to REPLANNING state when tool execution fails', () => {
    ...
    const run = plannerRunStore.findActive('user_replan_001').find((r) => r.plannerRunId === plannerRunId)
  ...
  it('limits consecutive replans and degrades to FAILED after repeated cycles', () => {
    ...
    for (let i = 0; i < failureReasons.length; i++) {
      ...
      let currentRun: PlannerRunRecord | null =
        plannerRunStore.findActive('user_replan_001').find((r) => r.plannerRunId === plannerRunId) ?? null
      ...
      if (i < failureReasons.length - 1) {
        ...
        currentRun = plannerRunStore.findActive('user_replan_001').find((r) => r.plannerRunId === plannerRunId) ?? null
  ...
  it('generates new ExecutionPlan version when replan adds revised steps', () => {
    ...
    expect(plannerRunStore.findActive('user_replan_001').find((r) => r.plannerRunId === plannerRunId)?.status).toBe(
