...
describe.skipIf(!hasDatabase)('PostgreSQL CRUD Operations', () => {
  ...
  describe('CREATE', () => {
    ...
    it('creates a table and inserts a row', async () => {
      ...
      await adapter.asyncExec(
        'INSERT INTO _test_crud (name, value) VALUES ($1, $2)',
        ['alpha', 100]
      );
    ...
    it('inserts multiple rows and returns row count', async () => {
      ...
      const rows = await adapter.asyncQuery<{ count: string }>(
        'SELECT COUNT(*)::text as count FROM _test_crud'
      );
  ...
  describe('READ', () => {
    ...
    it('reads all rows from table', async () => {
      const rows = await adapter.asyncQuery<{ name: string }>(
        'SELECT name FROM _test_crud ORDER BY id'
      );
    ...
    it('returns empty array when no rows match', async () => {
      const rows = await adapter.asyncQuery<{ name: string }>(
        'SELECT name FROM _test_crud WHERE name = $1',
        ['nonexistent']
      );
  ...
  describe('UPDATE', () => {
    ...
    it('updates existing rows with parameterized values', async () => {
      await adapter.asyncExec(
        'UPDATE _test_crud SET value = $1 WHERE name = $2',
        [999, 'alpha']
      );
    ...
    it('update affects only matching rows', async () => {
      await adapter.asyncExec(
        'UPDATE _test_crud SET value = $1 WHERE value > $2',
        [0, 15]
      );
  ...
  describe('DELETE', () => {
    ...
    it('deletes existing rows by parameterized condition', async () => {
      await adapter.asyncExec(
        'DELETE FROM _test_crud WHERE name = $1',
        ['beta']
      );
    ...
    it('delete removes only matching rows', async () => {
      await adapter.asyncExec(
        'DELETE FROM _test_crud WHERE value >= $1',
        [20]
      );
  ...
  describe('DDL', () => {
    ...
    it('drops and recreates table', async () => {
      await adapter.asyncExec(
        'CREATE TABLE _test_crud (id SERIAL PRIMARY KEY, val TEXT)'
      );
  ...
  describe('Adapter Interface', () => {
    ...
    it('sync methods throw DatabaseAdapterError', () => {
      const { DatabaseAdapterError } = require('../../../src/storage/database-adapter.js') as typeof import('../../../src/storage/database-adapter.js');
