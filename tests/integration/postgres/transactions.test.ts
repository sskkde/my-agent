...
describe.skipIf(!hasDatabase)('PostgreSQL Transactions', () => {
  ...
  it('commits successful transaction', async () => {
    await adapter.asyncTransaction(async () => {
      await adapter.asyncExec(
        'INSERT INTO _test_txn (name, value) VALUES ($1, $2)',
        ['committed', 1]
      );
    });

    const rows = await adapter.asyncQuery<{ name: string }>(
      'SELECT name FROM _test_txn'
    );
  ...
  it('rolls back on constraint violation', async () => {
    await adapter.asyncExec(
      'ALTER TABLE _test_txn ADD CONSTRAINT _test_txn_unique_name UNIQUE (name)'
    );
    await adapter.asyncExec(
      "INSERT INTO _test_txn (name, value) VALUES ('unique_name', 1)"
    );

    await expect(
      adapter.asyncTransaction(async () => {
        await adapter.asyncExec(
          "INSERT INTO _test_txn (name, value) VALUES ('unique_name', 2)"
        );
      })
    ).rejects.toThrow();
  ...
  it('multiple operations in a single transaction are atomic', async () => {
    await adapter.asyncTransaction(async () => {
      ...
      await adapter.asyncExec(
        'UPDATE _test_txn SET value = $1 WHERE name = $2',
        [99, 'first']
      );
    });
  ...
  it('partial rollback does not affect committed data', async () => {
    await adapter.asyncExec(
      "INSERT INTO _test_txn (name, value) VALUES ('pre_existing', 1)"
    );
    ...
    const rows = await adapter.asyncQuery<{ name: string }>(
      'SELECT name FROM _test_txn ORDER BY id'
    );
    expect(rows).toHaveLength(1);
  ...
  it('transaction return value is propagated', async () => {
    const result = await adapter.asyncTransaction(async () => {
      ...
      const rows = await adapter.asyncQuery<{ value: number }>(
        'SELECT value FROM _test_txn WHERE name = $1',
        ['return_test']
      );
      return rows[0].value;
    });
  ...
  it('concurrent transactions are isolated (READ COMMITTED)', async () => {
    ...
    try {
      ...
      const result2 = await client2.query(
        'SELECT name FROM _test_txn WHERE name = $1',
        ['uncommitted']
      );
