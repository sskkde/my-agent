...
describe.skipIf(!hasDatabase)('PostgreSQL Concurrent Access', () => {
  ...
  it('handles multiple concurrent inserts', async () => {
    ...
    const promises = Array.from({ length: CONCURRENT_COUNT }, (_, i) =>
      adapter.asyncExec(
        'INSERT INTO _test_concurrent (val, label) VALUES ($1, $2)',
        [i, `row_${i}`]
      )
    );
    await Promise.all(promises);

    const rows = await adapter.asyncQuery<{ val: number }>(
      'SELECT val FROM _test_concurrent ORDER BY val'
    );
  ...
  it('handles concurrent reads and writes', async () => {
    await adapter.asyncExec(
      "INSERT INTO _test_concurrent (val, label) VALUES (0, 'initial')"
    );
  ...
  it('serializes conflicting updates on same row', async () => {
    ...
    try {
      ...
      await client1.query(
        "UPDATE _test_concurrent SET val = $1 WHERE label = 'counter'",
        [val1 + 10]
      );
  ...
  it('handles concurrent deletes on different rows', async () => {
    ...
    const rows = await adapter.asyncQuery<{ label: string }>(
      'SELECT label FROM _test_concurrent ORDER BY label'
    );
