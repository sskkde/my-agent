    ...
    it('no file imports from prompt-builder.js', () => {
      ...
      if (violations.length > 0) {
        const formatted = violations
          ...
          .join('\n');
        throw new Error(
          `Found ${violations.length} file(s) importing from prompt-builder:\n${formatted}`
        );
      }
    ...
    it('no file imports from prompt-registry.js', () => {
      ...
      if (violations.length > 0) {
        const formatted = violations
          ...
          .join('\n');
        throw new Error(
          `Found ${violations.length} file(s) importing from prompt-registry:\n${formatted}`
        );
      }
  ...
  describe('No Legacy Function References', () => {
    it('no file contains buildRoutingMessages', () => {
      ...
      if (violations.length > 0) {
        ...
        throw new Error(
          `Found ${violations.length} file(s) containing 'buildRoutingMessages':\n${formatted}`
        );
      }
    ...
    it('no file contains getPromptForAgent', () => {
      ...
      if (violations.length > 0) {
        const formatted = violations.map((v) => `  - ${v.file}:${v.line}`).join('\n');
        throw new Error(
          `Found ${violations.length} file(s) containing 'getPromptForAgent':\n${formatted}`
        );
      }
