...
import { DEFAULT_SAFETY_POLICY } from '../../src/observability/replay.js'
import { DEFAULT_REPLAY_SAFETY_POLICY, createReplaySafetyGuard } from '../../src/replay/replay-safety-guard.js'
import type { ReplaySafetyPolicy, ReplaySafetyCheckResult } from '../../src/replay/replay-safety-guard.js'
...
describe('Path 7: Replay Preview Safety Contract', () => {
  ...
  describe('ReplayService — Replay Modes', () => {
    ...
    it('ReplayRequest carries mode and safety policy', () => {
      const reqKeys: Array<keyof ReplayRequest> = ['rootType', 'rootId', 'replayMode', 'safetyPolicy']
  ...
  describe('BlockedAction Type — Tracking Forbidden Operations', () => {
    it('BlockedAction tracks which action was blocked and why', () => {
      const blockedKeys: Array<keyof BlockedAction> = ['eventId', 'eventType', 'action', 'reason', 'module']
  ...
  describe('ReplayService — Config Dependencies', () => {
    it('ReplayServiceConfig requires timeline builder and stores (no tool executor)', () => {
      const configKeys: Array<keyof ReplayServiceConfig> = ['timelineBuilder', 'eventStore', 'auditStore', 'traceStore']
    ...
    it('StateSnapshot captures workflow, background, and planner state', () => {
      const snapshotKeys: Array<keyof StateSnapshot> = ['workflowRun', 'backgroundRun', 'plannerRun', 'timestamp']
  ...
  describe('ReplaySafetyPolicy ↔ SafetyPolicy Alignment', () => {
    ...
    it('SafetyPolicy extends ReplaySafetyPolicy with additional controls', () => {
      ...
      const extraFields: Array<keyof SafetyPolicy> = ['allowToolExecution', 'allowConnectorAccess', 'maxReplayDepth']
