  ...
  describe('DispatchRequest → RuntimeAction → DispatchResult Types', () => {
    it('RuntimeAction extends StorageRuntimeAction with actionType and policy', () => {
      const requiredKeys: Array<keyof RuntimeAction> = [
        'actionId', 'actionType', 'targetRuntime', 'source',
      ...
      const optionalKeys: Array<keyof RuntimeAction> = [
        'targetAction', 'userId', 'sessionId', 'targetRef',
        'payload', 'correlationId', 'causationId', 'idempotencyKey',
        'policy', 'statusMessage', 'result',
      ];
    ...
    it('DispatchResult documents all possible dispatch outcomes', () => {
      ...
      const optionalResultKeys: Array<keyof DispatchResult> = [
        'targetResultRef', 'result', 'waitingState', 'idempotency',
        'error', 'trace', 'completedAt',
      ];
    ...
    it('DispatchStatus covers all outcomes: accepted through cancelled', () => {
      const expectedStatuses: DispatchStatus[] = [
        'accepted', 'completed', 'queued', 'waiting_for_approval',
        'denied', 'duplicate', 'failed', 'timeout', 'cancelled',
      ];
    ...
    it('RuntimeActionType includes agent_run, tool, workflow, approval, and query operations', () => {
      ...
      const requiredActionTypes: RuntimeActionType[] = [
        'start_agent_run', 'execute_tool', 'start_workflow_run',
        'request_approval', 'spawn_planner_run', 'cancel_planner_run',
        'launch_subagent', 'query_active_work',
      ];
    ...
    it('TargetRuntime maps to 14 distinct runtime planes', () => {
      const expectedTargets: TargetRuntime[] = [
        ...
        'workflow_runtime', 'planner_runtime',
      ];
    ...
    it('DispatchFailureCode covers all known failure scenarios', () => {
      const expectedCodes: DispatchFailureCode[] = [
        'invalid_action', 'target_runtime_unavailable',
        'permission_denied', 'approval_required',
        'idempotency_duplicate', 'timeout',
        'queue_full', 'concurrency_limited',
        'target_runtime_error', 'policy_violation',
        'cancelled', 'target_state_invalid',
      ];
  ...
  describe('Dispatch Event Contract', () => {
    it('DispatchEventType covers the full dispatch lifecycle', () => {
      const expectedTypes: DispatchEventType[] = [
        'dispatch_requested', 'dispatch_accepted', 'dispatch_queued',
        'dispatch_started', 'dispatch_completed', 'dispatch_failed',
        'dispatch_denied', 'dispatch_waiting_approval',
        'dispatch_duplicate', 'dispatch_cancelled',
      ];
    ...
    it('DispatchEvent contains required observability fields', () => {
      const requiredFields: Array<keyof DispatchEvent> = [
        'eventId', 'eventType', 'actionId', 'requestId',
        'sourceModule', 'targetRuntime', 'actionType',
        'timestamp', 'createdAt', 'sensitivity', 'retentionClass',
      ];
      for (const f of requiredFields) {
        expect(typeof f).toBe('string');
      }
