...
import type { KernelConfig, KernelRunResult, ToolUseRequest } from '../../src/kernel/types.js'
  ...
  return {
    ...
    handler: async (params) => {
      ...
      return {
        ...
        data: {
          query,
          results: [{ title: 'Result', url: 'https://example.com', snippet: `Found: ${query}` }],
          totalResults: 1,
        },
...
describe('Flow: Tool Loop Closure (E2E)', () => {
  ...
  describe('Scenario 1: Full loop closure', () => {
    it('completes tool call → result → LLM text response cycle', async () => {
      ...
      const toolCalls: ToolCall[] = [makeToolCall('call-search-1', 'web_search', { query: 'TypeScript performance' })]
    ...
    it('returns LLM response from read-tool dispatch without ack message', async () => {
      ...
      const toolCalls: ToolCall[] = [makeToolCall('call-read-1', 'file_read', { path: '/tmp/test.txt' })]
  ...
  describe('Scenario 2: toolCallSummaries visible in transcript API', () => {
    it('persists toolCallSummaries in transcript when tool loop completes', async () => {
      ...
      const toolCalls: ToolCall[] = [makeToolCall('call-ts-1', 'web_search', { query: 'weather today' })]
      ...
      const transcript: TurnTranscript = {
        ...
        output: {
          visibleMessages: [{ messageId: 'msg-ts-1', role: 'assistant', content: result.finalResponse ?? '' }],
        },
        runtimeSummary: {
          toolCallSummaries: result.toolCalls.map((tc: ToolUseRequest) => ({
      ...
      expect(savedTranscript.runtimeSummary?.toolCallSummaries![0]).toEqual({
    ...
    it('omits runtimeSummary from transcript when not provided', async () => {
      const transcript: TurnTranscript = {
        ...
        output: {
          visibleMessages: [{ messageId: 'msg-off-1', role: 'assistant', content: 'Hello!' }],
        },
        visibility: 'public',
        createdAt: new Date().toISOString(),
      }
  ...
  describe('Scenario 3: Multiple parallel tools paired correctly (EC-5)', () => {
    it('pairs 3 parallel tool calls and stores all results', async () => {
      ...
      const transcript: TurnTranscript = {
        ...
        output: {
          visibleMessages: [{ messageId: 'msg-ec5-1', role: 'assistant', content: result.finalResponse ?? '' }],
        },
        runtimeSummary: {
          toolCallSummaries: result.toolCalls.map((tc: ToolUseRequest) => ({
            toolCallId: tc.toolCallId,
    ...
    it('detects orphan results via PairingGuard', async () => {
      ...
      const toolCalls: ToolCall[] = [makeToolCall('call-orp-1', 'web_search', { query: 'test' })]
    ...
    it('detects missing results via PairingGuard', async () => {
      ...
      const toolCalls: ToolCall[] = [makeToolCall('call-mis-1', 'web_search', { query: 'test' })]
  ...
  describe('Edge cases', () => {
    ...
    it('handles tool execution error gracefully', async () => {
      ...
      const toolCalls: ToolCall[] = [makeToolCall('call-err-1', 'web_search', { query: 'fail' })]
    ...
    it('completes multi-turn tool loop with multiple tool rounds', async () => {
      ...
      const toolCalls1: ToolCall[] = [makeToolCall('call-mt-1', 'web_search', { query: 'step 1' })]
      const toolCalls2: ToolCall[] = [makeToolCall('call-mt-2', 'file_read', { path: '/tmp/result.txt' })]
