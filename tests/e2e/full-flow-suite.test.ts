...
describe('Task 50: Full End-to-End Flow Suite', () => {
  ...
  describe('Flow 5: Background Task', () => {
    it('should complete background task with full observability', async () => {
      ...
      // Assert events
      const events = harness.stores.eventStore.query({ sessionId })
      const bgEvents = events.filter(
        (e: EventRecord) => e.eventType.includes('Background') || e.eventType.includes('background'),
      )
  ...
  describe('Flow 7: Event Trigger Wakeup', () => {
    it('should fire schedule trigger and create runtime action', async () => {
      ...
      const triggerEvents = events.filter(
        (e: EventRecord) => e.eventType.includes('trigger') || e.eventType.includes('Trigger'),
      )
  ...
  describe('Flow 9: Interrupt and Cancel', () => {
    it('should cancel running background task and create cancellation artifacts', async () => {
      ...
      const subagentSpan = tracingHooks.onSubagentRun(traceContext.traceId, 'data_analyzer', traceContext.rootSpanId)
      ...
      const cancelledEvent = events.find(
        (e: EventRecord) => e.eventType === 'BackgroundRunCancelled' || e.eventType === 'background_run_cancelled',
      )
  ...
  describe('Suite Summary and Evidence', () => {
    it('should generate suite completion evidence', async () => {
      ...
      fs.writeFileSync(path.join(evidenceDir, 'task-50-full-e2e.txt'), JSON.stringify(flowResults, null, 2))
