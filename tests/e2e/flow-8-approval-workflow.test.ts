...
describe('Flow 8: Approval Workflow E2E Flows', () => {
  ...
  describe('Approval Request Creation in Workflow', () => {
    ...
    it('should emit workflow_step_executing for approval step', () => {
      ...
      const executingEvent = events.find((e: EventRecord) => e.eventType === 'workflow_step_executing')
  ...
  describe('Event Audit Trail', () => {
    it('should create audit trail for approval workflow', () => {
      ...
      const draftCreated = events.some((e: EventRecord) => e.eventType === 'workflow_draft_created')
      const draftValidated = events.some((e: EventRecord) => e.eventType === 'workflow_draft_validated')
      const definitionPublished = events.some((e: EventRecord) => e.eventType === 'workflow_definition_published')
      const runStarted = events.some((e: EventRecord) => e.eventType === 'workflow_run_started')
      const stepExecuting = events.some((e: EventRecord) => e.eventType === 'workflow_step_executing')
