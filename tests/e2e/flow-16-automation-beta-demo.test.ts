...
describe('Flow 16: Automation Beta Demo', () => {
      ...
      const steps: WorkflowStep[] = [
        {
          ...
          stepType: 'tool_call',
          name: 'Connector Step',
          config: { toolName: 'connector_tool' },
        },
      ]
  ...
  describe('Step 7: Query observability console for run timeline', () => {
    it('should build timeline for a workflow run', () => {
      ...
      const timelineEventTypes = timeline.events
        .map((e) => (e.sourceData as { eventType?: string })?.eventType)
        .filter(Boolean)
  ...
  describe('Step 9: Query DLQ (empty after successful flow)', () => {
    ...
    it('should be able to enqueue and then resolve DLQ records', () => {
      ...
      const record = dlq.enqueue('workflow', 'test-event-001', 'Test DLQ entry for verification', { test: true })
