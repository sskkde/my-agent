...
describe('Flow 1: Ordinary Chat', () => {
  ...
  it('should NOT create RuntimeAction for ordinary chat', async () => {
    ...
    const chatRelatedActions = actions.filter(
      (a) => a.sessionId === sessionId && a.targetAction !== 'gateway.inbound_received',
    )
  ...
  it('should create Gateway events for chat flow', async () => {
    ...
    const inboundEvents = events.filter((e) => (e as { eventType: string }).eventType === 'gateway_inbound_received')
