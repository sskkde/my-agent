...
describe('Phase 3 E2E: Personal Assistant Core', () => {
  ...
  beforeEach(() => {
    ...
    registerConnectorTools(registry, { ...emailInstance, connectorId: 'gmail' }, emailCapabilities, {
    ...
    registerConnectorTools(registry, { ...docsInstance, connectorId: 'docs' }, docsCapabilities, {
      runtime: connectorRuntime,
    })
  })
  ...
  describe('Connector tool calls', () => {
    it('should emit connector events when instances are created', () => {
      ...
      const instanceEvents = events.filter((e) => e.eventType === 'connector_instance_created')
      expect(instanceEvents.length).toBeGreaterThan(0)
    })

    it('should emit connector events when definitions are registered', () => {
      const events = eventStore.query({})
      const definitionEvents = events.filter((e) => e.eventType === 'connector_definition_registered')
  ...
  describe('Audit output', () => {
    ...
    it('should include user and session context in audit events', async () => {
      ...
      const contextEvent = events.find((e) => e.eventType === 'BackgroundRunStarted' && e.userId === userId)
    ...
    it('should track event correlation and causation', async () => {
      ...
      const workflowEvents = events.filter(
        (e) => e.relatedRefs?.workflowRunId === run.workflowRunId || e.eventType?.startsWith('workflow_'),
      )
