...
function createReadTool(
  ...
  return {
    ...
    description:
      resultSize === 'large'
        ? 'Read a file, returns large output as resultRef'
        : 'Read a file, returns inline output',
    category: 'read',
    sensitivity: 'medium',
    schema: {
      type: 'object',
    ...
    handler: async (params, context) => {
      ...
      if (resultSize === 'large') {
        ...
        for (let i = 0; totalSize < INLINE_THRESHOLD + 2048; i++) {
          const content =
            `Line ${i + 1}: ${'x'.repeat(200)} - additional padding to reach threshold`;
...
describe('Flow 13: Read Tool + ResultRef Dedicated E2E', () => {
  ...
  describe('Assistant response structure', () => {
    ...
    it('sendMessage flow records tool executions with resultRef', async () => {
      ...
      const sessionExecs =
        harness.stores.toolExecutionStore.getBySession(sessionId);
  ...
  describe('Timeline events for tool execution', () => {
    it('creates timeline events when tool is executed', async () => {
      ...
      const toolEvents = events.filter(
        (e) =>
          e.eventType === 'tool_execution_completed' ||
          e.eventType?.startsWith('tool_'),
      );
      expect(toolEvents.length).toBeGreaterThan(0);

      // At least one event references the tool call
      const matchingEvent = toolEvents.find(
        (e) => e.payload?.toolCallId === toolCallId,
      );
    ...
    it('timeline includes tool execution event for read tools', async () => {
      ...
      const toolCompletedEvents = events.filter(
        (e) => e.eventType === 'tool_execution_completed',
      );
