...
describe('P6 Product Journey', () => {
  ...
  async function createApiKey(cookie: string, name: string, role: 'admin' | 'user' | 'service'): Promise<{ keyId: string; rawKey: string }> {
  ...
  describe('Step 2: API Key Lifecycle', () => {
    ...
    it('listing API keys does not expose full key or hash', async () => {
      ...
      expect(response.statusCode).toBe(200);
      const body = response.json() as { ok: boolean; data: Array<{ id: string; name: string; prefix: string; isActive: boolean }> };
      expect(body.ok).toBe(true);
  ...
  describe('Step 4: Trigger Creation', () => {
    it('creates a schedule trigger', async () => {
      ...
      expect(response.statusCode).toBe(201);
      const body = response.json() as { ok: boolean; data: { scheduleId: string; name: string; schedulePattern: string; status: string } };
      expect(body.ok).toBe(true);
    ...
    it('creates a webhook trigger with secret', async () => {
      ...
      expect(response.statusCode).toBe(201);
      const body = response.json() as { ok: boolean; data: { webhookId: string; name: string; status: string; secret: string; secretLast4: string } };
      expect(body.ok).toBe(true);
