...
function createBaseState(userId: string, sessionId: string, overrides?: Partial<ForegroundSessionState>): ForegroundSessionState {
  return {
    hydratedSession: {
...
describe('Flow 12: Planner Complex Task', () => {
  ...
  describe('Deterministic Plan Generation', () => {
    ...
    it('generates only valid step kinds', () => {
      const generator = new DeterministicPlanGenerator();
      const validKinds = ['agent_task', 'tool_call', 'subagent_task', 'workflow_step', 'user_approval', 'final_response'];
  ...
  describe('Full E2E Planner Pipeline', () => {
    ...
    it('full pipeline: route → create planner run → plan with steps → approval → actions → events', async () => {
      ...
      const writeSteps = genOutput.plan.steps.filter(
        s => s.kind === 'tool_call' && s.approvalRequirementId
      );
