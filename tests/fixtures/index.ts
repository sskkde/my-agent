...
export interface PlanFixture {
  ...
  sessionId?: string;
  objective: string;
  status: 'draft' | 'approved' | 'in_execution' | 'blocked' | 'waiting_for_user' | 'waiting_for_approval' | 'replanning' | 'completed' | 'failed' | 'abandoned';
...
export function createSessionFixture(overrides: Partial<SessionFixture> & { sessionId: string; userId: string }): SessionFixture {
  const now = new Date().toISOString();
  return {
    status: 'active',
    createdAt: now,
...
export function createTranscriptFixture(overrides: Partial<TranscriptFixture> & { turnId: string; sessionId: string; userId: string }): TranscriptFixture {
  const now = new Date().toISOString();
  return {
    input: {
      userMessageSummary: 'Default test message'
...
export function createToolResultFixture(overrides: Partial<ToolResultFixture> & { resultRef: string; toolCallId: string; toolName: string }): ToolResultFixture {
  const now = new Date().toISOString();
  return {
    userId: 'user_001',
    sessionId: undefined,
...
export function createApprovalFixture(overrides: Partial<ApprovalFixture> & { approvalId: string; userId: string }): ApprovalFixture {
  const now = new Date().toISOString();
  return {
    sessionId: undefined,
    status: 'pending',
...
export function createBackgroundRunFixture(overrides: Partial<BackgroundRunFixture> & { backgroundRunId: string; userId: string }): BackgroundRunFixture {
  const now = new Date().toISOString();
  return {
    sessionId: undefined,
    status: 'running',
