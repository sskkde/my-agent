...
function makeTestTemplates(): Map<string, PromptTemplateRecord> {
  return new Map([
    ['platform:base', {
      ...
      providerFamily: '*',
      layer: 1,
      content: 'You are a foreground routing agent. Your task is to classify the user message and decide the appropriate route.',
...
export function createMockModelInputBuilder(modeOverride?: 'routing_json' | 'routing_tool_call'): ModelInputBuilder {
  ...
  const build = vi.fn(async (input: ModelInputBuildInput): Promise<BuiltModelInput> => {
    ...
    const toolPlaneContent = toolIds.length > 0
    ...
    const messages = [
      { role: 'system' as const, content: 'You are a foreground routing agent. Your task is to classify the user message and decide the appropriate route.' },
