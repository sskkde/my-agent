...
function makeToolDef(name: string, category: ToolCategory, sensitivity: ToolSensitivity = 'low') {
  return {
    name,
...
describe('High-risk tools absent by default projection', () => {
  ...
  it('default projection includes only safe categories (read, search, internal)', () => {
    ...
    expect(projection.allowedToolIds).toEqual(expect.arrayContaining(['file_read', 'web_search', 'ask_user']))
