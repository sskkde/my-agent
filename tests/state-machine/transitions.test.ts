...
describe('Transition Validation', () => {
  describe('validateForegroundTransition', () => {
    it('should allow valid transitions', () => {
      const result = validateForegroundTransition(FOREGROUND_STATES.RECEIVED, FOREGROUND_STATES.HYDRATING)
      expect(result.valid).toBe(true)
      expect(result.error).toBeNull()
    })

    it('should reject invalid transitions from terminal states', () => {
      const result = validateForegroundTransition(FOREGROUND_STATES.COMPLETED, FOREGROUND_STATES.HYDRATING)
  ...
  describe('validatePlannerTransition', () => {
    it('should allow transition from initializing to planning', () => {
      const result = validatePlannerTransition(PLANNER_STATES.INITIALIZING, PLANNER_STATES.PLANNING)
      expect(result.valid).toBe(true)
    })

    it('should allow transition from planning to waiting_for_execution_result', () => {
      const result = validatePlannerTransition(PLANNER_STATES.PLANNING, PLANNER_STATES.WAITING_FOR_EXECUTION_RESULT)
      expect(result.valid).toBe(true)
    })

    it('should allow transition from planning to completed', () => {
      const result = validatePlannerTransition(PLANNER_STATES.PLANNING, PLANNER_STATES.COMPLETED)
      expect(result.valid).toBe(true)
    })

    it('should reject transition from completed to running', () => {
      const result = validatePlannerTransition(PLANNER_STATES.COMPLETED, PLANNER_STATES.PLANNING)
      expect(result.valid).toBe(false)
      expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
    })

    it('should reject transition from archived to any state', () => {
      const result = validatePlannerTransition(PLANNER_STATES.ARCHIVED, PLANNER_STATES.INITIALIZING)
      expect(result.valid).toBe(false)
    })
  })
  ...
  describe('validateExecutionPlanTransition', () => {
    it('should allow draft to approved', () => {
      const result = validateExecutionPlanTransition(EXECUTION_PLAN_STATES.DRAFT, EXECUTION_PLAN_STATES.APPROVED)
      expect(result.valid).toBe(true)
    })

    it('should allow approved to in_execution', () => {
      const result = validateExecutionPlanTransition(EXECUTION_PLAN_STATES.APPROVED, EXECUTION_PLAN_STATES.IN_EXECUTION)
      expect(result.valid).toBe(true)
    })

    it('should reject approved directly to completed', () => {
      const result = validateExecutionPlanTransition(EXECUTION_PLAN_STATES.APPROVED, EXECUTION_PLAN_STATES.COMPLETED)
      expect(result.valid).toBe(false)
  ...
  describe('validateRuntimeActionTransition', () => {
    it('should allow created to validated', () => {
      const result = validateRuntimeActionTransition(RUNTIME_ACTION_STATES.CREATED, RUNTIME_ACTION_STATES.VALIDATED)
      expect(result.valid).toBe(true)
    })

    it('should allow validated to accepted', () => {
      const result = validateRuntimeActionTransition(RUNTIME_ACTION_STATES.VALIDATED, RUNTIME_ACTION_STATES.ACCEPTED)
      expect(result.valid).toBe(true)
    })

    it('should allow validated to duplicate (terminal)', () => {
      const result = validateRuntimeActionTransition(RUNTIME_ACTION_STATES.VALIDATED, RUNTIME_ACTION_STATES.DUPLICATE)
      expect(result.valid).toBe(true)
    })

    it('should reject transition from duplicate to any state', () => {
      const result = validateRuntimeActionTransition(RUNTIME_ACTION_STATES.DUPLICATE, RUNTIME_ACTION_STATES.CREATED)
      expect(result.valid).toBe(false)
      expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
    })
  })
  ...
  describe('validateKernelRunTransition', () => {
    it('should allow initializing to building_context', () => {
      const result = validateKernelRunTransition(KERNEL_RUN_STATES.INITIALIZING, KERNEL_RUN_STATES.BUILDING_CONTEXT)
      expect(result.valid).toBe(true)
    })
  ...
  describe('validateToolExecutionTransition', () => {
    ...
    it('should reject transition from denied to executing', () => {
      const result = validateToolExecutionTransition(TOOL_EXECUTION_STATES.DENIED, TOOL_EXECUTION_STATES.EXECUTING)
      expect(result.valid).toBe(false)
    })
  })
  ...
  describe('validateBackgroundSubagentTransition', () => {
    ...
    it('should allow running to terminal states', () => {
      expect(
        validateBackgroundSubagentTransition(BACKGROUND_SUBAGENT_STATES.RUNNING, BACKGROUND_SUBAGENT_STATES.COMPLETED)
          .valid,
      ).toBe(true)
      expect(
        validateBackgroundSubagentTransition(BACKGROUND_SUBAGENT_STATES.RUNNING, BACKGROUND_SUBAGENT_STATES.FAILED)
          .valid,
      ).toBe(true)
    })
  ...
  describe('validateWorkflowRunTransition', () => {
    it('should allow queued to running', () => {
      const result = validateWorkflowRunTransition(WORKFLOW_RUN_STATES.QUEUED, WORKFLOW_RUN_STATES.RUNNING)
      expect(result.valid).toBe(true)
    })

    it('should allow running to paused', () => {
      const result = validateWorkflowRunTransition(WORKFLOW_RUN_STATES.RUNNING, WORKFLOW_RUN_STATES.PAUSED)
      expect(result.valid).toBe(true)
    })

    it('should allow paused to running', () => {
      const result = validateWorkflowRunTransition(WORKFLOW_RUN_STATES.PAUSED, WORKFLOW_RUN_STATES.RUNNING)
      expect(result.valid).toBe(true)
    })
  })
  ...
  describe('validateApprovalTransition', () => {
    it('should allow pending to approved', () => {
      const result = validateApprovalTransition(APPROVAL_STATES.PENDING, APPROVAL_STATES.APPROVED)
      expect(result.valid).toBe(true)
    })

    it('should allow pending to rejected', () => {
      const result = validateApprovalTransition(APPROVAL_STATES.PENDING, APPROVAL_STATES.REJECTED)
      expect(result.valid).toBe(true)
    })

    it('should allow pending to expired', () => {
      const result = validateApprovalTransition(APPROVAL_STATES.PENDING, APPROVAL_STATES.EXPIRED)
      expect(result.valid).toBe(true)
    })

    it('should allow pending to cancelled', () => {
      const result = validateApprovalTransition(APPROVAL_STATES.PENDING, APPROVAL_STATES.CANCELLED)
      expect(result.valid).toBe(true)
    })

    it('should reject all transitions from approved', () => {
      const result = validateApprovalTransition(APPROVAL_STATES.APPROVED, APPROVAL_STATES.REJECTED)
      expect(result.valid).toBe(false)
    })

    it('should reject all transitions from rejected', () => {
      const result = validateApprovalTransition(APPROVAL_STATES.REJECTED, APPROVAL_STATES.APPROVED)
      expect(result.valid).toBe(false)
    })
  })
  ...
  describe('validateWaitConditionTransition', () => {
    it('should allow registered to active', () => {
      const result = validateWaitConditionTransition(WAIT_CONDITION_STATES.REGISTERED, WAIT_CONDITION_STATES.ACTIVE)
      expect(result.valid).toBe(true)
    })

    it('should allow active to satisfied', () => {
      const result = validateWaitConditionTransition(WAIT_CONDITION_STATES.ACTIVE, WAIT_CONDITION_STATES.SATISFIED)
      expect(result.valid).toBe(true)
    })

    it('should allow active to timeout', () => {
      const result = validateWaitConditionTransition(WAIT_CONDITION_STATES.ACTIVE, WAIT_CONDITION_STATES.TIMEOUT)
      expect(result.valid).toBe(true)
    })

    it('should reject transition from satisfied to active', () => {
      const result = validateWaitConditionTransition(WAIT_CONDITION_STATES.SATISFIED, WAIT_CONDITION_STATES.ACTIVE)
      expect(result.valid).toBe(false)
    })
  })
  ...
  describe('validateTriggerEventTransition', () => {
    it('should allow created to matched', () => {
      const result = validateTriggerEventTransition(TRIGGER_EVENT_STATES.CREATED, TRIGGER_EVENT_STATES.MATCHED)
      expect(result.valid).toBe(true)
    })

    it('should allow matched to action_created', () => {
      const result = validateTriggerEventTransition(TRIGGER_EVENT_STATES.MATCHED, TRIGGER_EVENT_STATES.ACTION_CREATED)
      expect(result.valid).toBe(true)
    })
    ...
    it('should allow dispatched to handled', () => {
      const result = validateTriggerEventTransition(TRIGGER_EVENT_STATES.DISPATCHED, TRIGGER_EVENT_STATES.HANDLED)
      expect(result.valid).toBe(true)
    })

    it('should allow matched to duplicate (terminal)', () => {
      const result = validateTriggerEventTransition(TRIGGER_EVENT_STATES.MATCHED, TRIGGER_EVENT_STATES.DUPLICATE)
      expect(result.valid).toBe(true)
    })
  })
  ...
  describe('validateSummaryTransition', () => {
    it('should allow candidate to validated', () => {
      const result = validateSummaryTransition(SUMMARY_STATES.CANDIDATE, SUMMARY_STATES.VALIDATED)
      expect(result.valid).toBe(true)
    })

    it('should allow validated to active', () => {
      const result = validateSummaryTransition(SUMMARY_STATES.VALIDATED, SUMMARY_STATES.ACTIVE)
      expect(result.valid).toBe(true)
    })

    it('should allow active to superseded', () => {
      const result = validateSummaryTransition(SUMMARY_STATES.ACTIVE, SUMMARY_STATES.SUPERSEDED)
      expect(result.valid).toBe(true)
    })

    it('should allow active to archived', () => {
      const result = validateSummaryTransition(SUMMARY_STATES.ACTIVE, SUMMARY_STATES.ARCHIVED)
      expect(result.valid).toBe(true)
    })
  })
  ...
  describe('validateMemoryTransition', () => {
    it('should allow candidate to validated', () => {
      const result = validateMemoryTransition(MEMORY_STATES.CANDIDATE, MEMORY_STATES.VALIDATED)
      expect(result.valid).toBe(true)
    })

    it('should allow validated to active', () => {
      const result = validateMemoryTransition(MEMORY_STATES.VALIDATED, MEMORY_STATES.ACTIVE)
      expect(result.valid).toBe(true)
    })

    it('should allow active to various terminal states', () => {
      expect(validateMemoryTransition(MEMORY_STATES.ACTIVE, MEMORY_STATES.ARCHIVED).valid).toBe(true)
      expect(validateMemoryTransition(MEMORY_STATES.ACTIVE, MEMORY_STATES.COMPRESSED).valid).toBe(true)
      expect(validateMemoryTransition(MEMORY_STATES.ACTIVE, MEMORY_STATES.LOW_PRIORITY).valid).toBe(true)
    })
  })
})
