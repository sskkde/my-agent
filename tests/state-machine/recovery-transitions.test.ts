...
describe('Recovery Transitions', () => {
  describe('Terminal State Recovery Constraints', () => {
    ...
    describe('PlannerRun', () => {
      it('should reject transition from completed to initializing', () => {
        const result = validatePlannerTransition(PLANNER_STATES.COMPLETED, PLANNER_STATES.INITIALIZING)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })

      it('should reject transition from failed to initializing', () => {
        const result = validatePlannerTransition(PLANNER_STATES.FAILED, PLANNER_STATES.INITIALIZING)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })

      it('should reject transition from cancelled to initializing', () => {
        const result = validatePlannerTransition(PLANNER_STATES.CANCELLED, PLANNER_STATES.INITIALIZING)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })
    ...
    describe('WorkflowRun', () => {
      it('should reject transition from completed to running', () => {
        const result = validateWorkflowRunTransition(WORKFLOW_RUN_STATES.COMPLETED, WORKFLOW_RUN_STATES.RUNNING)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })

      it('should reject transition from failed to running', () => {
        const result = validateWorkflowRunTransition(WORKFLOW_RUN_STATES.FAILED, WORKFLOW_RUN_STATES.RUNNING)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })

      it('should reject transition from cancelled to running', () => {
        const result = validateWorkflowRunTransition(WORKFLOW_RUN_STATES.CANCELLED, WORKFLOW_RUN_STATES.RUNNING)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })

      it('should reject transition from timeout to running', () => {
        const result = validateWorkflowRunTransition(WORKFLOW_RUN_STATES.TIMEOUT, WORKFLOW_RUN_STATES.RUNNING)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })
    })
    ...
    describe('KernelRun', () => {
      it('should reject transition from completed to initializing', () => {
        const result = validateKernelRunTransition(KERNEL_RUN_STATES.COMPLETED, KERNEL_RUN_STATES.INITIALIZING)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })

      it('should reject transition from failed to initializing', () => {
        const result = validateKernelRunTransition(KERNEL_RUN_STATES.FAILED, KERNEL_RUN_STATES.INITIALIZING)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })

      it('should reject transition from cancelled to initializing', () => {
        const result = validateKernelRunTransition(KERNEL_RUN_STATES.CANCELLED, KERNEL_RUN_STATES.INITIALIZING)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })
    })
    ...
    describe('ForegroundConversationRun', () => {
      it('should reject transition from completed to received', () => {
        const result = validateForegroundTransition(FOREGROUND_STATES.COMPLETED, FOREGROUND_STATES.RECEIVED)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })

      it('should reject transition from failed to received', () => {
        const result = validateForegroundTransition(FOREGROUND_STATES.FAILED, FOREGROUND_STATES.RECEIVED)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })
    })
    ...
    describe('RuntimeAction', () => {
      it('should reject transition from completed to created', () => {
        const result = validateRuntimeActionTransition(RUNTIME_ACTION_STATES.COMPLETED, RUNTIME_ACTION_STATES.CREATED)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })

      it('should reject transition from failed to created', () => {
        const result = validateRuntimeActionTransition(RUNTIME_ACTION_STATES.FAILED, RUNTIME_ACTION_STATES.CREATED)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })

      it('should reject transition from cancelled to created', () => {
        const result = validateRuntimeActionTransition(RUNTIME_ACTION_STATES.CANCELLED, RUNTIME_ACTION_STATES.CREATED)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })
    })
    ...
    describe('ToolExecution', () => {
      it('should reject transition from completed to received', () => {
        const result = validateToolExecutionTransition(TOOL_EXECUTION_STATES.COMPLETED, TOOL_EXECUTION_STATES.RECEIVED)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })

      it('should reject transition from failed to received', () => {
        const result = validateToolExecutionTransition(TOOL_EXECUTION_STATES.FAILED, TOOL_EXECUTION_STATES.RECEIVED)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })

      it('should reject transition from cancelled to received', () => {
        const result = validateToolExecutionTransition(TOOL_EXECUTION_STATES.CANCELLED, TOOL_EXECUTION_STATES.RECEIVED)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })
    })
  ...
  describe('Recovery Creates New Run (Not Modifies Terminal)', () => {
    it('should enforce that terminal states are immutable', () => {
      ...
      terminalStates.forEach((state) => {
        const result = validateBackgroundSubagentTransition(state, BACKGROUND_SUBAGENT_STATES.RUNNING)
        expect(result.valid).toBe(false)
        expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
      })
    })
    ...
    it('should allow recovery only from non-terminal states', () => {
      ...
      nonTerminalStates.forEach((state) => {
        const result = validateBackgroundSubagentTransition(state, BACKGROUND_SUBAGENT_STATES.RECOVERING)
  ...
  describe('Idempotent Write Actions on Recovery', () => {
    ...
    it('should prevent write action duplication by blocking terminal-to-active transitions', () => {
      const result = validateRuntimeActionTransition(RUNTIME_ACTION_STATES.COMPLETED, RUNTIME_ACTION_STATES.DISPATCHING)
      expect(result.valid).toBe(false)
      expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
    })

    it('should prevent duplicate execution by blocking completed-to-executing transition', () => {
      const result = validateToolExecutionTransition(TOOL_EXECUTION_STATES.COMPLETED, TOOL_EXECUTION_STATES.EXECUTING)
      expect(result.valid).toBe(false)
      expect(result.error?.code).toBe('INVALID_FROM_TERMINAL')
    })
  })
