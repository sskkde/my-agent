...
import { handleThink, handleVerbose, handleReasoning, preferenceHandlers } from '../preferences.js'
...
describe('Preference Command Handlers', () => {
  ...
  describe('handleThink', () => {
    ...
    it('should set thinking level to valid values', async () => {
      ...
      for (const level of levels) {
        ...
        expect(result.output?.content).toContain('UI preference only')
        expect(mockSavePreferences).toHaveBeenCalledWith(expect.objectContaining({ thinkingLevel: level }))
      }
    })
    ...
    it('should handle case-insensitive values', async () => {
      ...
      expect(result.success).toBe(true)
      expect(mockSavePreferences).toHaveBeenCalledWith(expect.objectContaining({ thinkingLevel: 'medium' }))
    })
  ...
  describe('handleVerbose', () => {
    ...
    it('should enable verbose mode with on', async () => {
      ...
      expect(result.success).toBe(true)
      expect(result.output?.content).toContain('Verbose output: on')
      expect(mockSavePreferences).toHaveBeenCalledWith(expect.objectContaining({ verbose: true }))
    })
    ...
    it('should disable verbose mode with off', async () => {
      ...
      expect(result.success).toBe(true)
      expect(result.output?.content).toContain('Verbose output: off')
      expect(mockSavePreferences).toHaveBeenCalledWith(expect.objectContaining({ verbose: false }))
    })
  ...
  describe('handleReasoning', () => {
    ...
    it('should enable reasoning display with on', async () => {
      ...
      expect(mockSavePreferences).toHaveBeenCalledWith(expect.objectContaining({ reasoningVisible: true }))
    })
    ...
    it('should disable reasoning display with off', async () => {
      ...
      expect(mockSavePreferences).toHaveBeenCalledWith(expect.objectContaining({ reasoningVisible: false }))
    })
