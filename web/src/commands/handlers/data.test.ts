...
describe('dataHandlers', () => {
  ...
  describe('handleLogs', () => {
    it('should get logs with default filters', async () => {
      const mockLogs = [
        { eventId: 'e1', eventType: 'user_message', sourceModule: 'gateway', severity: 'info', summary: 'Test', createdAt: new Date().toISOString() },
      ];
    ...
    it('should filter logs by --session flag', async () => {
      const mockLogs = [{ eventId: 'e1', eventType: 'test', sourceModule: 'mod', severity: 'info', summary: 's', createdAt: new Date().toISOString() }];
      (client.getLogs as Mock).mockResolvedValue({ logs: mockLogs, total: 1 });
    ...
    it('should filter logs by --event-type flag', async () => {
      const mockLogs = [{ eventId: 'e1', eventType: 'user_message', sourceModule: 'mod', severity: 'info', summary: 's', createdAt: new Date().toISOString() }];
      (client.getLogs as Mock).mockResolvedValue({ logs: mockLogs, total: 1 });
    ...
    it('should filter logs by --source flag', async () => {
      const mockLogs = [{ eventId: 'e1', eventType: 'test', sourceModule: 'gateway', severity: 'info', summary: 's', createdAt: new Date().toISOString() }];
      (client.getLogs as Mock).mockResolvedValue({ logs: mockLogs, total: 1 });
  ...
  describe('handleExportSession', () => {
    ...
    it('should export selected session transcripts', async () => {
      const mockTranscripts = [
        { turnId: 't1', sessionId: 'session-123', userId: 'u1', input: {}, output: { visibleMessages: [] }, visibility: 'public', createdAt: new Date().toISOString() },
      ];
    ...
    it('should export specified session transcripts', async () => {
      const mockTranscripts = [
        { turnId: 't1', sessionId: 'specified', userId: 'u1', input: {}, output: { visibleMessages: [] }, visibility: 'public', createdAt: new Date().toISOString() },
      ];
      (client.getTranscripts as Mock).mockResolvedValue({ transcripts: mockTranscripts, total: 1 });
