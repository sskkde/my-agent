...
export const handleThink: CommandHandler = async (
  args: string[]
  ...
  if (args.length === 0) {
    return {
      success: true,
      output: { type: 'text', content: `Current thinking level: ${formatThinkingLevel(prefs.thinkingLevel)}\n\nThis is a UI preference only; backend model behavior is unchanged.` },
      commandName: 'think',
    };
  }
  ...
  if (!VALID_THINKING_LEVELS.includes(value as ThinkingLevel)) {
    return {
      success: false,
      output: { type: 'error', content: `Invalid thinking level: "${args[0]}". Valid values are: off, minimal, low, medium, high.` },
  ...
  return {
    success: true,
    output: { type: 'text', content: `Thinking level set to: ${formatThinkingLevel(value as ThinkingLevel)}\n\nThis is a UI preference only; backend model behavior is unchanged.` },
    commandName: 'think',
  };
};

export const handleVerbose: CommandHandler = async (
  args: string[]
): Promise<FrontendCommandResult> => {
  const prefs = loadPreferences();
  ...
  if (args.length === 0) {
    return {
      success: true,
      output: { type: 'text', content: `Verbose output: ${prefs.verbose ? 'on' : 'off'}\n\nThis is a UI preference only.` },
      commandName: 'verbose',
    };
  }
...
export const handleReasoning: CommandHandler = async (
  args: string[]
): Promise<FrontendCommandResult> => {
  const prefs = loadPreferences();
