...
import { getHealth, getTools, getProviders, logout as apiLogout } from '../../api/client.js'
...
export const handleStatus: CommandHandler = async (
  ...
  try {
    ...
    return {
      ...
      output: {
        type: 'error',
        content: `Failed to get system status: ${error instanceof Error ? error.message : String(error)}`,
      },
...
export const handleDiagnostics: CommandHandler = async (
  ...
  try {
    ...
    return {
      ...
      output: {
        type: 'error',
        content: `Diagnostics failed: ${error instanceof Error ? error.message : String(error)}`,
      },
...
export const handleTools: CommandHandler = async (
  ...
  try {
    ...
    return {
      ...
      output: {
        type: 'error',
        content: `Failed to get tools: ${error instanceof Error ? error.message : String(error)}`,
      },
  ...
  try {
    ...
    if (providers.length === 0) {
      return {
        ...
        output: {
          type: 'text',
          content: 'No providers configured. Configure a provider first to see available models.',
        },
        commandName: 'models',
      }
    }
    ...
    return {
      ...
      output: {
        type: 'error',
        content: `Failed to get models: ${error instanceof Error ? error.message : String(error)}`,
      },
...
export const handleModel: CommandHandler = async (
  ...
  return {
    ...
    output: {
      type: 'error',
      content: `Model selection validation not yet implemented. Attempted to select: ${modelArg}`,
    },
