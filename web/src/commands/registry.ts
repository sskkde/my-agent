...
export class CommandRegistry {
  ...
  register(
    command: FrontendCommandDefinition,
  ...
  registerCatalog(
    catalog: Record<string, FrontendCommandDefinition>,
    ...
    for (const [name, definition] of Object.entries(catalog)) {
      ...
      if (!handler) {
        throw new CommandRegistryError(
          `No handler provided for command: ${name}`
        );
      }
  ...
  getCommand(name: string): RegisteredCommand | null {
    ...
    const finalDefinition = this.definitions.get(resolvedName) ??
      getCommand(resolvedName);
