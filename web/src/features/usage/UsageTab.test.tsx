...
describe('UsageTab', () => {
  ...
  it('displays aggregate cards with correct data', async () => {
    const mockUsages = [
      createMockUsage({ messageCount: 10, toolCallCount: 3, approvalCount: 1, estimatedTotalTokens: 1500, estimatedCostCents: 100 }),
      createMockUsage({ messageCount: 20, toolCallCount: 5, approvalCount: 2, estimatedTotalTokens: 2500, estimatedCostCents: 200 }),
    ];
  ...
  it('displays "未配置" for null cost', async () => {
    const mockUsages = [
      createMockUsage({ estimatedCostCents: null }),
    ];
  ...
  it('displays per-session usage table', async () => {
    const mockUsages = [
      createMockUsage({ sessionId: 'session-abc123xyz', messageCount: 10, estimatedTotalTokens: 1500, estimatedCostCents: 100 }),
      createMockUsage({ sessionId: 'session-def456uvw', messageCount: 20, estimatedTotalTokens: 2500, estimatedCostCents: 200 }),
    ];
  ...
  it('displays "未配置" for null cost in table', async () => {
    const mockUsages = [
      createMockUsage({ sessionId: 'session-abc123xyz', estimatedCostCents: null }),
    ];
  ...
  it('displays pagination when there are multiple pages', async () => {
    const mockUsages = Array.from({ length: 10 }, (_, i) =>
      createMockUsage({ sessionId: `session-${i}` })
    );
  ...
  it('truncates session IDs to 12 characters', async () => {
    const mockUsages = [
      createMockUsage({ sessionId: 'very-long-session-id-that-needs-truncation' }),
    ];
