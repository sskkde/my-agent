...
describe('GA: AdminTab Error/Loading/Empty States', () => {
  ...
  it('retry button triggers data reload', async () => {
    ...
    vi.mocked(adminApi.listApiKeys)
      .mockRejectedValueOnce(new Error('API error'))
      .mockResolvedValueOnce({ keys: [], total: 0 })
    vi.mocked(adminApi.getConnectorHealth)
      .mockRejectedValueOnce(new Error('API error'))
      .mockResolvedValueOnce({ connectors: [] })
    vi.mocked(adminApi.getSystemSettings)
      .mockRejectedValueOnce(new Error('API error'))
...
describe('GA: ConnectorsTab Error/Loading/Empty States', () => {
  ...
  it('retry button triggers data reload', async () => {
    vi.mocked(connectorsApi.getConnectors).mockRejectedValueOnce(new Error('Network error')).mockResolvedValueOnce([])
