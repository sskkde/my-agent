...
export const ProcessingStatus: React.FC<ProcessingStatusProps> = ({
  ...
  const model = hasActiveProcessing && processingStatus.providerId && processingStatus.model
    ? `${processingStatus.providerId}/${processingStatus.model}`
    : null;
  
  const stage = hasActiveProcessing && processingStatus.stageLabel
    ? processingStatus.stageLabel
    : null;
  
  const context = hasActiveProcessing && processingStatus.contextUsage
  ...
  const tools = hasActiveProcessing && processingStatus.activeTools?.length
    ? processingStatus.activeTools.map(t => t.toolId).join(', ')
    : null;

  const error = hasActiveProcessing && processingStatus.error
