...
describe('SessionsTab', () => {
  ...
  it('displays loading state', async () => {
    (client.getSessions as ReturnType<typeof vi.fn>).mockImplementation(
      () => new Promise(() => {})
    );
  ...
  it('hides archive button for archived sessions', async () => {
    ...
    expect(
      screen.queryByTestId('session-archive-button-session-2')
  ...
  it('hides close button for closed sessions', async () => {
    ...
    expect(
      screen.queryByTestId('session-close-button-session-3')
    ).not.toBeInTheDocument();
  });
});
