...
function makeDraft(overrides: Partial<WorkflowDraftResponse> = {}): WorkflowDraftResponse {
  return {
    ...
    name: 'Test WF',
    description: '',
    steps: [{ stepId: 's1', stepType: 'tool_call' as const, name: 'Step 1', config: { toolName: 'status.query' } }],
    ownerUserId: 'user-1',
    status: 'draft',
    validationIssues: [],
...
function makeRun(overrides: Partial<WorkflowRunResponse> = {}): WorkflowRunResponse {
  return {
    ...
    status: 'pending',
    currentStepIds: ['s1'],
    stepRuns: [{ stepRunId: 'sr-1', stepId: 's1', stepType: 'tool_call', status: 'pending' }],
    ...overrides,
  }
}
