...
const ProductionSetupChecklist: React.FC<ProductionSetupChecklistProps> = ({ onComplete }) => {
  ...
  const checkProductionReadiness = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        const hasUnhealthy = connectorHealth.connectors.some(
          c => c.status === 'unhealthy'
        );
        const hasDegraded = connectorHealth.connectors.some(
          c => c.status === 'degraded'
        );
  ...
  const renderStepIndicator = () => (
    <div className="setup-steps-indicator">
      {steps.map((step, index) => (
        <div
          key={step.id}
          className={`setup-step-item ${step.status} ${currentStep === index ? 'active' : ''}`}
        >
  ...
  const renderAdminUserStep = () => (
    <div className="setup-form-container">
      ...
      <p className="setup-form-subtitle">
        这是平台的第一个账户，将自动获得管理员权限
      </p>
      
      <form className="setup-form" onSubmit={handleCreateAdmin}>
        {error && (
          <ErrorMessage error={error} size="small" data-testid="setup-admin-error" />
        )}
  ...
  const renderApiKeyStep = () => (
    <div className="setup-form-container">
      ...
      <p className="setup-form-subtitle">
        API 密钥用于程序化访问平台，如 CI/CD 集成、自动化脚本等
      </p>
      ...
      {createdApiKey ? (
        <div className="api-key-success">
          ...
          <div className="api-key-info">
            <p><strong>名称:</strong> {createdApiKey.name}</p>
            <p><strong>前缀:</strong> {createdApiKey.prefix}</p>
            <p><strong>角色:</strong> {createdApiKey.role}</p>
          </div>
          <button
            className="setup-submit-button"
        ...
        <form className="setup-form" onSubmit={handleCreateApiKey}>
          {error && (
            <ErrorMessage error={error} size="small" data-testid="setup-api-key-error" />
          )}
            ...
            {readinessItems.map((item) => (
              ...
              >
                ...
                <div className="readiness-content">
                  ...
                  {item.details && (
                    <div className="readiness-details">{item.details}</div>
                  )}
                </div>
              </div>
            ))}
