...
- [ ] **Strict-Transport-Security 存在（HTTPS）**
  ```bash
...
- [ ] **Production Readiness Checklist 存在**
  ```bash
...
- [ ] **excludedPaths 覆盖几乎所有路由** — 已配置 API_AUTH_TOKEN 作为替代保护
- [ ] **CORS origin:true 允许所有来源** — 计划后续版本修复
- [ ] **速率限制豁免本地请求** — 开发便利，生产环境需注意
- [ ] **Cookie 未设置 Secure 标志** — 计划后续版本修复
- [ ] **会话认证不保护大多数端点** — 使用 API Key 认证作为主要保护
