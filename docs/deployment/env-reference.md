...
> 工作目录功能为每个用户提供隔离的文件存储空间。模型在选定工作目录内执行文件操作时无需审批；Shell 命令和代码执行仍需走审批流程。不支持访问工作目录之外的任意主机路径。
...
**⚠️ 持久化要求：** 此目录存储所有用户的工作目录文件。如果目录丢失或重建，用户工作目录中的文件将无法恢复。Docker 部署中默认路径已被 `agent_data` 卷覆盖；自定义路径时必须确保对应目录也做了卷挂载。

**目录结构：**
```
<WORKDIR_ROOT>/
  <userId>/
    <workdirId>/     # 用户的文件存储在这里
```
...
- 路径穿越（`../`）和符号链接逃逸会被拒绝
- 跨用户隔离由数据库层强制执行
- 文件操作在选定工作目录内免审批；执行工具仍需审批

**示例**：
```bash
WORKDIR_ROOT=/data/workdirs
```
...
### AUTO_CONTINUE_ON_BACKGROUND_COMPLETE

| 属性 | 值 |
|------|-----|
| **用途** | 后台（子代理）任务到达终态时，是否自动触发父会话一轮回复 |
| **默认值** | `true` |
| **可选值** | `true`, `false` |
| **生产要求** | 可选 |
...
- 当一个后台子任务进入终态（`completed` / `failed` / `cancelled`）时，平台自动在父会话中触发一轮回复，无需用户发送新消息即可感知结果。
- 自动回复**仅发送到 Web UI 通道**，不会推送到外部消息通道（飞书、Telegram、钉钉、QQ、企业微信等）。
- 自动轮次中**禁止启动新的子任务**（前台与后台均拒绝），防止递归。
- 用户消息正在处理时跳过自动触发，通知保持 pending，由当前轮结束后的 onIdle 单次重试或下次用户消息兜底。
- 自动回复失败时通知不会丢失：claim 回滚，通知保持 pending，在后续轮次重试。
- 合成轮次不写入用户消息文本，不污染会话历史与统计。

**关闭行为**：设置为 `false` 后，后台任务完成不会主动触发回复，结果仅在下次用户消息时通过现有通知注入机制感知。

**成本提示**：开启时，每个后台任务完成会消耗一次 LLM turn。后台任务频繁的场景需评估调用成本。

**示例**：
```bash
# 默认开启
AUTO_CONTINUE_ON_BACKGROUND_COMPLETE=true

# 关闭自动继续（结果仅在下次用户消息时感知）
AUTO_CONTINUE_ON_BACKGROUND_COMPLETE=false
```
